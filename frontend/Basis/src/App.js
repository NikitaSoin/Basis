...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      // /futures/, /funds/) ПРИЛОЖЕНИЕМ НЕ ПЕРЕХВАТЫВАЮТСЯ — СОЗНАТЕЛЬНО, ОТКАТ 2026-08-31.
      //
      // 26 августа я сделал наоборот: адрес подборки открывал «Рынок» с включённым
      // фильтром. Для человека стало лучше, для поиска — хуже, и владелец это заметил:
      // «после того как мы наладили страницу ВДО, она в поиске улетела на последние
      // позиции, хотя стала нормальной». Проверка подтвердила: из индекса страница не
      // выпадала (она там и сейчас), но визиты из поиска прекратились ровно в день
      // выкатки — было 1–2 в день, стало 0.
      //
      // Механизм. Перехват приложением заканчивается удалением блока #seo-static из
      // документа. Яндекс исполняет скрипты — и вместо страницы «высокодоходные
      // облигации: что это, кому подходит, чем рискуете» видит таблицу котировок без
      // текста. Страница перестаёт отвечать на запрос, по которому её находили.
      // Тот же механизм однажды уронил фьючерс из выдачи — про это написано выше.
      //
      // Совместить нельзя: показывать текст роботу и прятать от человека — подмена
      // контента, за неё наказывают. Оставить текст ПОД живым списком владелец не
      // захотел («если про саму платформу — чтобы туда докидать текста, то нет»).
      // Значит выбор бинарный, и он сделан в пользу поиска: /bonds/vdo/ была лучшей
      // точкой входа на всём сайте (19 визитов в месяц), терять её ради удобства
      // перехода, который и так есть в кнопке, — размен не в нашу пользу.
      //
      // Человек попадает в приложение кнопкой внизу страницы: она ведёт на
      // /?view=companies&tab=bonds&preset=<слаг>, фильтр включается (см. BOND_PRESETS
      // в market/MarketNeo.jsx). То есть UX-претензия закрыта, а текст остался.
      //
      // Ниже по коду адрес попадёт в ветку staticOnly и статика останется как есть —
      // это и есть нужное поведение, отдельная ветка тут не нужна.
