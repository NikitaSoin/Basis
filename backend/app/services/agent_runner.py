...
def run_agent(db: Session, *, system_prompt: str, task: str, tools_schema: list[dict],
    ...
    for step in range(1, max_steps + 1):
        ...
        try:
            ...
            # даём ему место.
            # 🔴 Потолок финала действует ВСЕГДА, а не только после «последнего
            # звонка». Прежнее условие давало его лишь при пустой схеме
            # инструментов — то есть агент, закончивший работу штатно (не упёршись
            # в бюджет), получал на итог всего step_max_tokens и обрывался на
            # середине JSON. Так упали два слоя из четырёх сразу после перевода на
            # агентский цикл: макро-выпуск и портрет очага.
            # Поднимать потолок безопасно: max_tokens — это ВЕРХНЯЯ ГРАНИЦА, а не
            # резервирование. Платим за сгенерированное, а короткие шаги остаются
            # короткими потому, что модель зовёт инструмент, а не потому, что мы
            # ей не даём места.
        ...
        # 🔴 Тот же звонок — по ШАГАМ, а не только по токенам. Найдено 2026-09-13
        # на первом прогоне институционального снимка: 14 шагов, 29 вызовов
        # инструментов, 603 тыс. токенов из 900 — бюджет не исчерпан, звонок не
        # прозвучал, лимит шагов кончился молча, и агент так и не получил просьбы
        # вернуть итог. Вся работа ушла в мусор по той же причине, что и раньше по
        # бюджету: остановка без запроса финала. Предпоследний шаг — всегда звонок.
        if not last_call_made and (tokens_used > max_tokens_total * 0.7
                                   or step >= max_steps - 2):
            last_call_made = True
            trace.append({"step": step, "event": "last_call", "tokens": tokens_used,
                          "why": "шаги" if step >= max_steps - 2 else "бюджет"})
