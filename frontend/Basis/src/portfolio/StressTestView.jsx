...
// Агрегатный индекс — headline-число консоли.
//
// 🔴 Считается ПО ДЕНЬГАМ, а не как среднее процентов (владелец, 2026-08-08:
// «из-за того что малая прибыль у Озона, у него рост на несколько сотен процентов, и
// потом в индексном числе сумасшедшие проценты»). Прежняя формула усредняла
// проценты компаний: у Озона прибыль около миллиарда, поэтому его Δ давала сотни
// процентов и одна бумага перекашивала весь рынок. Вес «голубые фишки ×3» этого не
// лечил — он менял вклад втрое, а разрыв был стократный.
//
// Теперь это «на сколько изменится СОВОКУПНАЯ прибыль рынка»: сумма изменений делится
// на сумму баз. Компания с крошечной базой вносит крошечный вклад — ровно столько,
// сколько весит в экономике, — а искусственные веса не нужны вовсе.
//
// База берётся по модулю: у убыточной компании отрицательная база иначе вычиталась бы
// из знаменателя и завышала итог.
function computeHeadlineIndex(companies) {
  let sumDelta = 0, sumBase = 0;
  for (const c of companies) {
    const np = c.metrics?.net_profit;
    if (np?.delta_bn == null || np?.base_bn == null) continue;
    sumDelta += np.delta_bn;
    sumBase += Math.abs(np.base_bn);
  }
  return sumBase > 0 ? (sumDelta / sumBase) * 100 : 0;
}
    ...
    <div className="st-headline">
      <div className="st-headline-num">
        ...
        <span className="st-hl-clarify">
          ...
          Изменение совокупной прибыли всех компаний вселенной: сумма изменений
          делится на сумму годовых баз. Не индекс Мосбиржи и не взвешивание по
          капитализации — компания весит ровно столько, сколько весит её прибыль.
        </span>
      </div>
