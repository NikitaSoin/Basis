...
_OUTPUT_SPEC = (
    ...
    "     \"why_names\": \"<ПОЧЕМУ именно эти бумаги: чем они отличаются от соседей по сектору — доля экспорта, долг, доля импортных издержек. Без этого список тикеров бесполезен, максимум 22 слова>\"}\n"
    "  ],\n"
    ...
    "B. ДОХОДЫ ЭКСПОРТЁРОВ = мировая цена МИНУС дисконт. В данных есть ряд дисконта "
    ...
    "H. ВАЖНОСТЬ СОБЫТИЯ оценивай САМ: масштаб (доля рынка/выпуска), стадия (нарастает, "
...
_KEY_FACT_SPECS = (
    ...
    ("urals_brent_spread", "level",
     "ДИСКОНТ Urals к Brent, $/барр. Рост мировой цены и доходы российских "
     "экспортёров — РАЗНЫЕ вещи, если дисконт одновременно расширяется"),
