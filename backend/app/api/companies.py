...
def get_overview_synthesis(ticker: str, db: Session = Depends(get_db),
    ...
    # 🔴 Объяснение справедливой цены следует за доступом к самой цене. Сейчас цена
    # открыта всем (владелец 2026-08-05: «справедливую цену и объяснение оставь в
    # бесплатном тарифе»), поэтому проверка ничего не срезает. Связка оставлена
    # намеренно: если доступ когда-нибудь сузят, объяснение закроется ВМЕСТЕ с ценой,
    # а не останется висеть под пустым местом — разбор «почему цена такая» без самой
    # цены бессмыслен и раздаёт платное содержание.
    # Общий вывод о бизнесе не зависит от этого: он не про цену.
