...
_OUTPUT_SPEC = (
    ...
    "     \"analysis\": \"<🔴 РАЗБОР, а не строчка: 4-6 предложений по методичке «макро → "
    "сектор». Обязательно: (1) через какую ОПЕРАЦИОННУЮ метрику сектора приходит эффект "
    "(продажи м², LFL, NIM и стоимость риска, добыча и спред, загрузка, тарифная выручка) "
    "и что с ней происходит СЕЙЧАС; (2) как это доходит до unit economics и P&L — маржа, "
    "процентные расходы, оборотный капитал; (3) с каким ЛАГОМ; (4) что меняет ПРОГНОЗ — "
    "при базовом пути ЦБ и при альтернативном, если для сектора это существенно. Числа — "
    "из входных данных, чужие прогнозы — с автором>\",\n"
    ...
    "  ],\n"    "  \"decomposition\": [ // 🔴 2-3 ключевые переменные (ставка, инфляция, ВВП) — "
    "разложение по методичке «институты → макроэкономика».\n"
    "    {\"variable\": \"Ключевая ставка|Инфляция|ВВП\",\n"
    "     \"cyclical\": \"<циклическая компонента: что объясняется фазой цикла и "
    "разрывом выпуска, максимум 20 слов>\",\n"
    "     \"shock\": \"<шоковая: какие конкретные события её создают, максимум 20 слов>\",\n"
    "     \"institutional\": \"<🔴 институциональная: КОНКРЕТНЫЙ механизм, а не "
    "«плохие институты». Например: льготные и административные кредитные контуры не "
    "реагируют на ставку, поэтому охлаждение достигается только за счёт рыночного "
    "сегмента и требует более высокой ставки. Максимум 32 слова>\",\n"
    "     \"policy_or_structure\": \"<что из этого устранимо текущей политикой, а что "
    "требует институционального изменения — и, значит, не уйдёт при смене ставки>\",\n"
    ...
    "7к3. 🔴 sectors[].analysis — ГЛАВНОЕ поле секторного блока, а не приписка. Одна "
    "фраза «ставка давит на девелоперов» не разбор: читатель это знает. Разбор — это "
    "путь от операционной метрики сектора к прибыли с лагом и с числом, по методичке "
    "«макро → сектор» (её каркас и строки «чем живёт сектор» приложены ниже). Если "
    "сказать по существу нечего — лучше НЕ включай сектор в список, чем включай пустым.\n"
    ...
    "сценарием её сюда не выноси — для неё есть свой раздел.\n"    "7н. 🔴 ВЕРОЯТНОСТИ СЦЕНАРИЕВ БЕРИ ИЗ snapshot.scenario_odds КАК ЕСТЬ (методичка, "
    "14.6). Их считает КОД по наблюдаемым сигналам — консенсус аналитиков против базового "
    "пути ЦБ, инфляционный импульс, кредит, нефть — и рядом лежит причина каждого сдвига. "
    "Твоя работа: объяснить эти веса словами и назвать причину, а не назначить свои. "
    "Своя вероятность, не совпадающая с расчётом платформы, — грубая ошибка. Называй их "
    "оценкой платформы: ЦБ даёт пути, веса наши.\n"    "8. Не делай разворотных выводов по одному месяцу (Часть 1): топливо, плодоовощи, "
...
def _scenario_odds(db: Session) -> dict:
    """Веса макро-сценариев с причинами — считает код (см. macro_scenario_odds)."""
    try:
        from app.services.macro_scenario_odds import compute
        return compute(db)
    except Exception:  # noqa: BLE001
        logger.warning("Интерпретатор: веса сценариев не посчитаны", exc_info=True)
        return {}
...
def _inst_macro_core() -> str:
    """Ядро методички «институты → макроэкономика».

    🔴 ЗАЧЕМ. Выпуск объясняет ПОВЕДЕНИЕ показателей, и до сих пор объяснял его
    циклом и шоками. Третья компонента — институциональная — не разбиралась вовсе,
    хотя именно она отвечает на давний вопрос владельца: почему ставка приходится
    держать выше, чем следовало бы из инфляции. Ответ методички конкретен:
    льготный и административный кредит, теневой сектор и бюджетозависимые
    заказчики на ставку не реагируют, поэтому вся тяжесть ложится на рыночный
    сегмент, и требуемая жёсткость выше.
    Берём операционное ядро: три канала входа, декомпозиция показателя, дисциплина
    атрибуции, модификаторы каналов ДКП, фискальное доминирование, инфляционная
    инерция, отток капитала как измеритель доверия, протоколы по трём целевым
    переменным, эталон вывода и чек-лист ошибок. Уравнения, секторальные балансы и
    кривая Лаффера остаются в документе — их открывает инструментом тот, кто
    считает конкретную задачу.
    """
    try:
        from app.services.methodology import core
    except ImportError:  # pragma: no cover — модуль новый, выкатка неравномерна
        logger.warning("Интерпретатор: полка методичек недоступна")
        return ""
    # 🔴 Список сверен по СОДЕРЖАНИЮ после доработки методички 2026-08-20, и это
    # было не формальностью: в Части 10 нумерация сдвинулась — под 10.2 теперь
    # «сценарное дерево», а чек-лист ошибок уехал на 10.3. Прежний список
    # продолжал НАХОДИТЬ раздел 10.2 и молча подсовывал бы модели не тот текст.
    # Добрано из новых разделов то, что меняет вывод, а не объём: доказательные
    # статусы (0.5), узкое место вместо средней оценки (0.6), диагностика
    # концентрации — тот случай, где высокая доля рынка сама по себе не диагноз
    # (2.9), финансовый цикл и накопление уязвимости (4.7), опережающие
    # индикаторы (9.2) и сценарное дерево с вероятностями ветвей (10.2).
    # 🔴 Список ужат по РАЗМЕРУ ПРОМПТА, а не по важности. Полный набор дал 24
    # тысячи знаков, и системный промпт выпуска дошёл до 131 тысячи — около
    # 44 тысяч токенов ДО того, как туда лягут снапшот и досье. Ответ выпуска и
    # так упирался в потолок дважды; наращивать вход, зная это, — значит идти на
    # тот же обрыв осознанно.
    # Оставлено то, что МЕНЯЕТ ВЫВОД: декомпозиция и эталон её подачи, дисциплина
    # атрибуции, модификаторы каналов ДКП (главный механизм), фискальное
    # доминирование, инфляционная инерция, узкое место вместо средней оценки,
    # диагностика концентрации, чек-лист ошибок.
    # Убрано, но НЕ потеряно: протоколы по ВВП/инфляции/ставке (форму уже задают
    # поле decomposition и эталон), отток капитала, финансовый цикл, опережающие
    # индикаторы, сценарное дерево, статусы F/D/I/H (у платформы свои эпистем-теги).
    # Всё это лежит на полке целиком, и разведчик открывает нужное инструментом —
    # ровно ради этого полка и делалась.
    return core("inst_macro",
                ["0.1", "0.2", "0.3", "0.6", "2.9", "4.3", "4.6", "5.2",
                 "8.5", "10.3"],
                header="===== МЕТОДИЧКА: ИНСТИТУТЫ → МАКРОЭКОНОМИКА =====\n"
                       "Ею разбирай, ПОЧЕМУ показатель ведёт себя так, а не иначе, "
                       "и что из этого лечится политикой, а что нет.")
...
def generate(db: Session) -> MacroInterpretation:
    ...
    system = (_methodology() + _inst_macro_core() + _OUTPUT_SPEC
              + (("\n\n" + bridge) if bridge else ""))

    # 🔴 РАЗВЕДКА ПЕРЕД ВЫПУСКОМ (владелец, 2026-08-12). Сквозная боль платформы —
    # устаревшие входные данные: Росстат машинно недоступен, часть фидов отстаёт.
    # Раньше интерпретатор работал ровно с тем, что лежало в снапшоте: нет свежего
    # значения — либо молчим, либо пишем общими словами, и пойти проверить, вышла
    # ли публикация, было нечем. Теперь агент с веб-доступом сначала СОБИРАЕТ:
    # находит устаревшее и оборванное, добирает из первоисточника, записывает
    # досье отдельной версией. В базу показателей он ничего не пишет — только в
    # текст выпуска со ссылкой (добытчик чисел уже показал, что без сверки вторым
    # источником его значения не воспроизводятся между прогонами).
    # Разведка НЕ обязательна: её сбой не должен гасить выпуск.
    dossier_text = ""
    try:
        ...
        try:
            prev = get_latest(db)
            prev_sum = str(getattr(prev, "summary", "") or "")[:1200] if prev else ""
        except Exception:  # noqa: BLE001
            prev_sum = ""
        dossier = macro_scout.run(db, snapshot, prev_summary=prev_sum)
        dossier_text = macro_scout.dossier_block(dossier)
    except Exception as e:  # noqa: BLE001
        logger.warning("Интерпретатор: разведка недоступна (%s) — выпуск по снапшоту", e)

    user = ((dossier_text + "\n\n" if dossier_text else "")
            + "Данные платформы на текущий момент (используй конкретные значения):\n\n"
