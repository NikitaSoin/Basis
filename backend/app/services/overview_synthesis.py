...
# 🔴 «рекоменд\w*» ловило своих: «совет директоров рекомендовал дивиденд»,
# «рекомендованная выплата отклонена собранием» — это корпоративная процедура, которую
# аналитик обязан описывать, а не наш совет инвестору. Запрещено то, что советует
# ПЛАТФОРМА, а не то, что рекомендует эмитент своим акционерам.
# 🔴 «продать/купить» описательно — можно (владелец 2026-09-13: «вынуждена продать
# актив» — не совет). Запрещён СОВЕТ: рекомендуем/стоит/пора купить-продать-держать,
# «купить акции», целевая цена, инвестидея.
_BANNED = re.compile(r"(?:стоит|следует|советуем|пора|надо|нужно|лучше|имеет смысл)\s+(?:\S+\s+){0,2}?"
                     r"(?:купить|покупать|докупить|продать|продавать|держать|шортить)|"
                     r"(?:купить|покупать|докупить|продать|продавать|шортить)\s+(?:акци|облига|бумаг|ОФЗ|па[ия]\b|фонд)|"
                     r"\bшорт\b|целев\w+ цен|"
                     r"мы\s+рекоменду|рекомендуем|наша\s+рекомендаци|"
                     r"рекоменд\w*[\s:—–-]+(?:\S+\s+){0,3}?(?:куп\w+|покуп\w+|прода\w+|держ\w+)|"
