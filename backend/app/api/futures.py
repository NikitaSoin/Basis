...
# 🔴 Рублёвая ставка НЕ константа: раньше здесь стояло 14.5 «сверено с CBR на
# ~2026-06», и это число попадало прямо в текст вердикта о справедливости
# базиса. ЦБ снизил ставку до 14,0% 24.07.2026 — вердикт считался от неверной
# базы. Берём из макро-ряда (ежедневный синк ЦБ), фолбэк — на случай пустой БД.
# Иностранные ставки и дивдоходность индекса пока остаются ориентирами: своих
# рядов по ним нет, это честно помечено ниже как приблизительные значения.
def _key_rate_rub() -> float:
    try:
        from app.services.bond_risk import current_key_rate
        return current_key_rate()
    except Exception:  # noqa: BLE001
        return 14.0


KEY_RATE_RUB = _key_rate_rub()
