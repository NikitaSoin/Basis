...
def build_corporate(db: Session, max_pages: int = 5) -> list[dict]:
    ...
    публичный и парсится.
    🔴 Горизонт: страница `/calendar/stocks/` (без пагинации) показывает только ~неделю —
    но у smart-lab ЕСТЬ пагинация `from_{DD.MM.YYYY}/page{N}/` (обнаружено 2026-07-12 по
    просьбе владельца, до этого читали только page1) — по 30 событий на страницу, реально
    доходит до ~4,5 месяцев вперёд (проверено вручную: page3-4 дают даты до конца ноября,
    включая дивиденды С СУММОЙ — «AKRN: последний день с дивидендом 235 руб» на 07.08.2026).
    max_pages=5 — с запасом сверх наблюдавшихся 4 «живых» страниц (page5+ на практике
    повторяют одно и то же — считаем это концом ленты, не ошибкой). При суточном пересборе
    события НАКАПЛИВАЮТСЯ (дедуп не плодит) → растёт форвард-календарь. Тикер сопоставляется
    с нашими компаниями для сектора/портфеля.
    ...
    отсечки с суммой ЖИВЬЁ — берём её отсюда, dedup_key совпадает с build_dividends
    ...
    for page_n in range(1, max_pages + 1):
        url = f"{_SMARTLAB_CAL}from_{today_str}/page{page_n}/"
        try:
            r = httpx.get(url, timeout=30, headers=_SL_HTTP, follow_redirects=True)
            r.raise_for_status()
            page_html = r.text
        except Exception as e:  # noqa: BLE001
            logger.warning("Календарь корпсобытий: smart-lab %s недоступен: %s", url, type(e).__name__)
            break
        n_rows = len(re.findall(r'<tr[^>]*>.*?\d{2}\.\d{2}\.\d{4}', page_html, re.S))
        html_pages.append(page_html)
        if n_rows < 25:  # последняя частично заполненная страница ленты — дальше не ходим
            break
...
def _classify_report_kind(text: str) -> str:
    ...
    if "рсбу" in t or "рпбу" in t:
        return "РСБУ"
    # «Операционные И финансовые результаты» (типовой заголовок smart-lab, напр.
    # «CHMF: Операционные и финансовые результаты за 1П 2026») — это ФИНОТЧЁТ с
    # операционной частью, не операционка: раньше слово «операцион» уводило его в
    # operational-путь, финансовые цифры (выручка/EBITDA/прибыль) терялись, а
    # дедуп ±4дн потом блокировал создание правильной финансовой записи (реальный
    # кейс Северстали, найден 2026-07-25).
    if "операцион" in t and "финанс" not in t:
