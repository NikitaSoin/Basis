...
function bondIsThin(b) {
  ...
  // Живая рыночная доходность — достаточное основание оставить страницу.
  if (typeof b.ytm === "number" && b.ytm > 0) return false;
  // Нет доходности — но есть параметры купона, значит страница может показать календарь
  // выплат с датами и суммами (couponScheduleBlock). Это уникальный контент, а не шаблон,
  // и закрывать такую страницу не за что.
  if (b.coupon_value && b.coupon_period && b.maturity_date
      && b.coupon_type !== "floater" && b.coupon_type !== "linker") return false;
  // Ни цены, ни купонной сетки — сказать по существу нечего, остаётся шаблон.
  return true;

  // 🔴 Отсечку выбирал ТРИЖДЫ, и каждый раз её опровергали данные, а не рассуждение:
  //  1) «ОФЗ + муни + листинг 1–2 с доходностью» (911 страниц) — выбрасывал RU000A108BD9,
  //     по которому у нас РЕАЛЬНЫЙ запрос в Вебмастере («когда будет погашение»);
  //  2) «есть YTM» (1929 страниц) — Метрика 04.08 показала живые запросы по ISIN
  //     неторгуемых выпусков; при этом 584 закрытых бумаги после наполнения уже
  //     показывают календарь выплат, то есть тонкими быть перестали;
  //  3) текущий: закрываем ТОЛЬКО то, где нет ни цены, ни купонной сетки.
  // Правило на будущее: критерий отбора проверять на бумагах с подтверждённым спросом,
  // а не на логике «это же неликвид, кому он нужен».
}
