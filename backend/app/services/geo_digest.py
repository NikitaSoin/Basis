...
_BATCH = 6          # меньше, чем раньше (12): даём LLM больше текста на статью
                    # (полный content.rendered/description, не куцый excerpt) —
                    # батч меньше, чтобы не упереться в max_tokens ответа
...
_DIGEST_SYS = (
    ...
    '|null, "title": "<заголовок на русском>", "summary": "<подробный пересказ 4-7 '
...
def _digest_batch(articles: list[dict]) -> list[dict]:
    ...
    try:
        res = complete(_DIGEST_SYS, json.dumps(payload, ensure_ascii=False),
                       json_mode=True, max_tokens=6000, temperature=0.3)
