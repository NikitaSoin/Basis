    ...
    for r in rows:
        ...
        for factor, tokens in sector_scope.items():
            ...
            if not any(t in sector_l for t in tokens):
                ...
                # 🔴 Найдено 2026-07-17, ОБЩИЙ фикс добавлен 2026-07-21:
                # тегированный effect_sign в commodity-факторе кодирует «эффект
                # при ТЕКУЩЕЙ цене относительно нейтрали аналитика», не
                # структурную чувствительность — get_company_exposures()
                # (factor_exposures.py) теперь сама переопределяет это на
                # структурный знак для явных производителей сырья (не только
                # нефтегаз — металлурги/химия/уголь/золото и т.п.), так что
                # exp["commodity"] сюда уже приходит исправленным ИЗ ИСТОЧНИКА
                # — правка ниже для нефтегаза избыточна, но безвредна (2.0→2.0),
                # оставлена как явная гарантия для ЭТОГО конкретного сценария
                # (не полагаться молча на то, что источник не откатят).
                exp["commodity"] = 2.0
