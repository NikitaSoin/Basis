...
# Обращения к редакции/аналитику: на карточке их быть не должно — она пишется ЧИТАТЕЛЮ.
_EDITORIAL = re.compile(
    r"(рекоменду[ею]м|советуем|следует обновить|требуется обновить|необходимо обновить)"
    r"[^.]{0,60}(данн|обзор|разбор|карточк|оценк)", re.I)
...
🔴 ПРО УСТАРЕВШИЕ ДАННЫЕ ПИШИ ЧИТАТЕЛЮ, А НЕ РЕДАКЦИИ. Нельзя «рекомендуем обновить
данные по рынкам» — карточку читает инвестор, а не аналитик платформы. Правильно:
«оценка объёма рынка относится к 2023 году, более свежих публичных данных нет —
величину стоит считать ориентиром». Констатируй факт устаревания, не давай поручений.
...
def rewrite_one(db: Session, ticker: str, tab: str, *, facts: str, reason: str,
    ...
    # 🔴 Служебное обращение к САМИМ СЕБЕ на витрине. Первый опубликованный прогон по
    # HIMCP закончился фразой «Рекомендуем обновить данные по рынкам при появлении
    # свежих отраслевых обзоров» — это не инвестрекомендация (комплаенс не нарушен),
    # но текст адресован аналитику, а читает его инвестор. Проверяем кодом: критик
    # такое видит не всегда, а формулировки наперечёт.
    if not notes and _EDITORIAL.search(challenger) and not _EDITORIAL.search(champion):
        notes.append("editorial_addressing")
...
def scout_dossier(db: Session, ticker: str, need: str, *, web_calls: int = 3) -> dict:
    ...
    try:
        ...
        shelf_txt = shelf_card(["code", "macro_base", "geo_macro", "geo_inst", "inst_env", "inst_macro",
                                "inst_geo", "macro_sector"])
