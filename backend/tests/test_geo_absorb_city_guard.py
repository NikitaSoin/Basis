...
def test_присоединение_идёт_только_со_стороны_фронта():
    ...
    # Точка сбоку от линии «пункт → фронт» (~9 км в сторону, между пунктом и
    # фронтом): «крышка» (2026-09-14/16) закрывает карман полосой шириной 0,4
    # зазора, но не больше 7 км в каждую сторону от якоря — дальше остаётся чисто.
    aside = Point(lon + 0.12, lat - 0.08)
    ...
    assert not filled.contains(aside), "закрашена территория далеко сбоку от линии присоединения"
    # контроль: без потолка окна вдоль фронта та же точка закрашивалась бы
    saved = (sync._CAP_MIN_KM, sync._CAP_MAX_KM)
    try:
        sync._CAP_MIN_KM, sync._CAP_MAX_KM = 20.0, 40.0
        wide = sync._absorb_overrides(isw, [village])
    finally:
        sync._CAP_MIN_KM, sync._CAP_MAX_KM = saved
    assert wide.contains(aside), "сценарий не воспроизводит поведение без потолка — тест ничего не доказывал бы"
