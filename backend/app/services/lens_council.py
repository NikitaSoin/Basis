...
LENSES: list[str] = ["geo_base", "geo_events", "geo_macro", "macro_geo", "geo_inst", "inst_geo",
                     "inst_env", "inst_macro", "macro_inst", "macro_base", "macro_sector"]
CONTOUR_OF: dict[str, str] = {
    "geo_base": "геополитика", "geo_events": "геополитика", "inst_env": "институты",
    "macro_base": "экономика", "macro_sector": "экономика",
...
Ответ — короткий и по форме: его читает не человек, а сведение вместе с ответами коллег.
"""
...
def lens_system(doc_id: str) -> str:
    ...
    head = (f"Ты — агент-методичка Basis «{title}» (контур: {contour}; id={doc_id}). Basis — независимая "
            f"аналитика для частного инвестора в РФ. Ты один из {len(LENSES)} агентов совета: у каждого своя "
            "методичка, и на любую задачу совет смотрит столькими же разными способами, а потом сводит. "
...
ROUTES: dict[str, list[list[str]]] = {
    "geo": [["geo_events", "geo_base"], ["geo_macro", "geo_inst"],
            ["macro_base", "inst_env", "macro_geo", "inst_geo", "inst_macro", "macro_inst", "macro_sector"]],
    "macro": [["macro_base"], ["macro_geo", "macro_inst"],
              ["geo_base", "geo_events", "inst_env", "geo_macro", "inst_macro", "geo_inst", "inst_geo", "macro_sector"]],
    "inst": [["inst_env"], ["inst_macro", "inst_geo"],
             ["geo_base", "geo_events", "macro_base", "geo_macro", "macro_geo", "geo_inst", "macro_inst", "macro_sector"]],
}
...
_SYNTH_SYSTEM = (
    "Ты — сведение совета Basis (независимая аналитика для частного инвестора в РФ). Агенты совета "
    "посмотрели на одну задачу каждый через свою методичку (три базовые: геополитика, экономика, "
    "институты; шесть связок между ними; классы событий; отрасли) и ответили короткой формой; затем "
    "ответили на вопросы друг друга. Твоя работа — не пересказать ответы, а собрать из них ОДНУ "
    "системную картину.\n\n"
    "🔴 ПРАВИЛА СВЕДЕНИЯ:\n"
