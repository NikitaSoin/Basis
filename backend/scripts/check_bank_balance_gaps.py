...
NOT_APPLICABLE = {
    ...
    # 2021 — отчётность закрыта решением ЦБ от 14.04.2022 (временное нераскрытие)
    "MBNK": {"gross_loans": [2020, 2021, 2022, 2023], "loan_provisions": [2020, 2021, 2022, 2023],
             "cash_and_equivalents": [2021]},
    ...
    # у этих трёх разбивка резерва не раскрывается: публикуются упрощённые формы ЦБ
    # либо «обобщённая» отчётность без примечаний (проверено добытчиками отдельно)
    "AVAN": {"gross_loans": [2021], "loan_provisions": [2021], "due_to_banks": [2021, 2022]},
    # валовые кредиты за 2020-2022 стояли оценкой и оказывались МЕНЬШЕ чистых (невозможно) —
    # убраны; публичные формы РСБУ разбивку «валовые/резерв» за эти годы не дают
    # за 2021 год отчётности по МСФО нет вовсе — разрыв в раскрытии между отчётом за 2020
    # год и следующим доступным (за 2023-й со сравнительными данными за 2022-й). Прежняя
    # «лесенка» 23 500 / 25 000 / 27 000 была интерполяцией, а не данными.
    "RDRB": {"loan_provisions": [2020, 2021, 2022], "gross_loans": [2020, 2021, 2022],
             "total_assets": [2021], "total_liabilities": [2021], "total_equity": [2021],
             "net_loans": [2021], "securities": [2021], "customer_deposits": [2021],
             "due_to_banks": [2021], "cash_and_equivalents": [2021]},
    "SVCB": {"loan_provisions": [2020]},
