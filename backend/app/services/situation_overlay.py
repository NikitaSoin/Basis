...
# 🔴 Инвестсовет — ТОЛЬКО как совет (владелец, 2026-09-13): «продать» и «купить»
# можно употреблять описательно («компания вынуждена продать активы», «нерезиденты
# продают бумаги»), запрещена РЕКОМЕНДАЦИЯ купить/продать/держать бумаги. Голое
# `\bпродать\b` отклонило целую институциональную сводку за фразу о вынужденной
# продаже активов, а «рекомендация» ловила «рекомендации Банка России».
_INVEST_ADVICE = (
    # «рекомендуем», «мы рекомендуем», «наша рекомендация» — платформа советует
    r"\bрекомендуем\b|мы\s+рекоменду|наш\w*\s+рекомендаци|"
    # «рекомендация/рекомендуем/рекомендуется … купить/продать/держать»
    r"рекоменд\w*[\s:—–-]+(?:\S+\s+){0,3}?(?:куп\w+|покуп\w+|прода\w+|держ\w+|шорт\w*|докуп\w+|фиксир\w+ прибыл)|"
    # «стоит/следует/пора/надо купить/продать/держать …»
    r"(?:стоит|следует|советуем|советую|пора|надо|нужно|лучше|имеет смысл|целесообразно|разумно)\s+"
    r"(?:\S+\s+){0,2}?(?:купить|покупать|докупить|продать|продавать|держать|шортить|зафиксировать прибыль)|"
    # «купить/продать акции/облигации/бумаги/…» — императив к сделке с бумагой
    r"(?:купить|покупать|докупить|продать|продавать|шортить)\s+(?:акци|облига|бумаг|ОФЗ|фьючерс|па[ия]\b|фонд)|"
    r"инвестиде[яи]|целев\w+\s+цен\w+\s+(?:акци|бумаг)"
)
