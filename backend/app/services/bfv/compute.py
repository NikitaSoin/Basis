...
def compute_bfv(fin: dict, gov: dict, inst: dict, barometer: dict, *,
    ...
    if upside_pct > 100:
        ...
        reliability = "low"
    if upside_pct < -50:
        # Растущая компания с низкими ТЕКУЩИМИ выплатами: дивидендная модель считает
        # ближние потоки, а не потенциал роста, и системно недооценивает такие бумаги
        # (Яндекс и т.п.). Честно помечаем — это ограничение метода, не вердикт «дорого».
        warnings.append("растущая/низкодивидендная компания — дивидендная модель "
                        "недооценивает будущий рост (считает текущие выплаты, §12); "
                        "смотрите на ожидаемую доходность, а не на абсолютную цену")
        reliability = "low"
    if tv_share > 0.85:
