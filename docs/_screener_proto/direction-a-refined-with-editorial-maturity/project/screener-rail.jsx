...
function CriterionRow({ mkey, range, onChange, onRemove, matchCount }) {
  ...
  let readout;
  if (atFloor && !atCeil) readout = "≤ " + fmtBound(mkey, hi);else
  if (!atFloor && atCeil) readout = "≥ " + fmtBound(mkey, lo);else
  readout = fmtBound(mkey, lo) + " – " + fmtBound(mkey, hi);
  return (
...
// Full rail.
function CriteriaRail({ ranges, sectors, onRangeChange, onAdd, onRemove, onToggleSector, onReset, resultCount }) {
  ...
  return (
    <aside className="sc-rail">
      <div className="sc-rail-head">
        <div>
          <div className="sc-eyebrow" data-comment-anchor="793efa3c5d-div-137-11">Критерии скрина</div>
      ...
      <div className="sc-rail-scroll">
        ...
        <div className="sc-empty">Фильтров нет — показаны все бумаги вселенной. Добавьте критерий или выберите готовый скрин.</div>
        }
        {window.SC_METRIC_GROUPS.map((g) => {
