...
_OUTPUT_SPEC = (
    ...
    "  \"scenarios\": \"<СЦЕНАРИИ base/bull/bear с триггерами и пометкой ‘оценка Basis’>\"\n"
    "}}. Каждый раздел — связный текст (можно абзацы через \\n). Опирайся на КОНКРЕТНЫЕ "
    "значения показателей из переданных данных. Тон спокойный, без ‘купить/продать’. "
