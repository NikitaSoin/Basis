...
SOURCE_LABELS = {
    ...
    # 🔴 Без подписи на витрине печатался служебный ключ («ecb» вместо «ЕЦБ»)
    # — владелец, 2026-08-09. Новый источник без строки здесь снова покажет ключ,
...
_DIGEST_SYS = (
    ...
    "1б. 🔴 ПЕРЕСКАЗЫВАЙ СОДЕРЖАНИЕ, А НЕ ФАКТ ПУБЛИКАЦИИ. Владелец, 2026-08-09: смысл "
    "раздела в том, чтобы инвестор НЕ читал доклад на 50 страниц, а получил из него "
    "главное. Запрещены формулировки вида «опубликованы данные о балансах и капитале», "
    "«эти данные включают информацию о...», «публикация таких данных важна для оценки», "
    "«инвесторы и аналитики используют эти данные» — это описание оглавления, из него "
    "читатель не узнаёт НИЧЕГО. Нужны сами величины и выводы: что показали цифры, куда "
    "они сдвинулись, что это меняет. Если в исходном тексте самих величин нет (анонс, "
    "оглавление, пресс-релиз без данных) — такую статью НЕ ПУБЛИКУЙ (publish=false), "
    "пустой пересказ хуже отсутствия карточки.\n"
    "1в. Приоритет — влияние на РОССИЙСКИЙ рынок. Материал зарубежного ЦБ (ФРС, ЕЦБ, "
    "Народный банк Китая, BIS) публикуй тогда, когда из него следует что-то для ставок, "
    "курса, сырья, аппетита к риску или для российских эмитентов, и скажи это прямо. "
    "Событие, интересное только внутри своей юрисдикции, пропускай.\n"    "6. investor_relevance — отдельно, 1-2 фразы: зачем инвестору это знать (на что обратить "
