...
# ── рубильник ────────────────────────────────────────────────────────────────
# 🔴 РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-08: «пока всё должно быть открыто, тарифы ничего не
# должны банить, платформа новая и тестируется». Одного env-рубильника оказалось мало:
# на бою TIER_LIMITS_ENFORCED=1 стоял в одиночку (фронтовая половина выключена), и
# платформа закрывала свой сценарий стресс-теста и разборы отчётов — причём даже
# подписчику Max, потому что экран стресс-теста вообще не слал токен и сервер видел
# гостя. Поэтому границы теперь требуют ЯВНОГО выпуска в коде, а не только переменной
# окружения, которую легко выставить в панели и забыть.
#
# ВКЛЮЧЕНИЕ (три вещи, снимать ВМЕСТЕ, иначе снова получится «ошибки вместо тарифа»):
#   1) LIMITS_ROLLOUT = True здесь;
#   2) env TIER_LIMITS_ENFORCED=1 на бэкенде;
#   3) FREE_LIMITS_ENFORCED = true во frontend/Basis/src/account/tierCatalog.js.
LIMITS_ROLLOUT = False
