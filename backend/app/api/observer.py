...
from app.models.observer_report import ObserverReport, REPORT_TYPES, TOPICS
...
def _serialize(r: ObserverReport) -> dict:
    return {
        "id": r.id, "report_type": r.report_type, "topic": r.topic, "horizon_days": r.horizon_days,
...
def create_report(type: str = Query("express"), topic: str = Query("mixed"),
                  db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """Сгенерировать сводный отчёт по типу (express|detailed|deep) и теме-фокусу
    (biz|macro|geo|institutions|mixed). Синтез по данным платформы (Напр.1-4,6,7 +
    портфель). Сохраняется в историю пользователя."""
    if type not in REPORT_TYPES:
        raise HTTPException(status_code=400, detail="Неизвестный тип отчёта")
    if topic not in TOPICS:
        raise HTTPException(status_code=400, detail="Неизвестная тема отчёта")
    from app.services.observer_report import generate
    from app.services.llm import LLMError
    try:
        rep = generate(db, user.id, type, topic)
...
def list_reports(limit: int = Query(30, ge=1, le=100),
    ...
    return [{"id": r.id, "report_type": r.report_type, "topic": r.topic, "horizon_days": r.horizon_days,
             "generated_at": r.generated_at.isoformat() if r.generated_at else None,
