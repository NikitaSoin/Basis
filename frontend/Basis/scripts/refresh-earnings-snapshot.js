...
async function main() {
  ...
  // Геополитический барометр — под блок «что происходит сейчас» на странице геополитики.
  // Она была тонкой (249 слов), а тонкую страницу поиск не поднимет, каким бы верным ни
  // был заголовок. Сценарные вероятности и разбор по очагам — то, чего нет ни у кого.
  await grab("Гео-барометр", "/api/market/geo-barometer",
    (d) => (d && (d.summary || d.regions)
      ? [{ as_of: d.as_of, summary: d.summary, regions: d.regions,
           scenarios: (d.handoff || {}).scenarios, barometer: d.barometer }] : []),
    "geo-barometer-snapshot.json");
