...
_TOOLS_FRAMEWORK = (
    ...
    "- ОТЧЁТНОСТЬ построчно по годам (выручка, себестоимость, долг, capex, денежные "
    "потоки, маржи; у крупнейших — кварталы и полугодия) — get_financial_statements. "
    "Числа карточки берутся отсюда, поэтому на вопрос про конкретную строку отвечай "
    "этим инструментом, а не пересказом прозы;\n"
    "- прогноз компании тремя сценариями (база/бык/медведь, драйверы, чувствительность) "
    "— get_financial_model;\n"
    "- среда рынка: геополитический и институциональный барометры — get_barometer; "
    "карта рынка, недооценённость, индексы IMOEX/RTSI/MCFTR — get_market_map;\n"
    "- «что будет, если ставка/нефть/курс…» — stress_test (с apply_to_portfolio=true "
    "считает по реальным позициям спрашивающего);\n"
    "- портфель СПРАШИВАЮЩЕГО — get_portfolio, его диагноз и индекс качества — "
    "get_portfolio_diagnosis (у гостя их нет, это нормально).\n"
    ...
    "(markdown), без JSON.\n"
    "🔴 «КАКИЕ ЕСТЬ» — ВСЕГДА ИНСТРУМЕНТ. Вопрос про состав, перечень, наличие или "
    "цену («какие фонды есть», «что у этого эмитента», «сколько стоит», «у кого "
    "доходность выше») требует вызова инструмента, даже если ты знаешь ответ из "
    "эрудиции: пользователь спрашивает про КОНКРЕТНЫЙ набор бумаг платформы и живые "
    "числа. Эрудиция — для механики и терминов (как устроен инструмент, чем классы "
    "активов отличаются друг от друга), состав и числа — только из данных платформы.\n"
...
_TOOL_REF_TITLES = {
    ...
    "get_financial_statements": "Отчётность компании по годам",
    "get_financial_model": "Финансовая модель — прогноз по сценариям",
    "get_barometer": "Барометр среды (Обозреватель)",
    "get_market_map": "Карта рынка и индексы",
    "stress_test": "Стресс-тест по сценарию",
    "get_portfolio_diagnosis": "ИИ-диагноз вашего портфеля",
}
...
_META_MARKERS = re.compile(
    ...
    r"сейчас проверю|вызываю инструмент|использую инструмент|собер[ую] (в )?(связн|единый|ответ)|"
    r"давайте (я )?(соберу|посмотрим)|"
