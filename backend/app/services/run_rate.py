...
def get_run_rate(db: Session, ticker: str) -> dict:
    ...
    # 🔴 ПРЕДОХРАНИТЕЛЬ «АНОМАЛЬНЫЙ БАЗОВЫЙ ГОД» (2026-09-11). Сезонная доля по ОДНОМУ
    # прошлому году — это не сезонность, а одна точка: если в том году период дал
    # необычно малую часть годового итога (разовые статьи, обвал процентными
    # расходами), деление на эту долю не продолжает темп, а ВОСПРОИЗВОДИТ аномалию.
    # Боевой случай MTSS: доля 1П в годовой прибыли 2025 — 19,9 %, отсюда прикидка
    # 381 млрд при 38,6 млрд прошлого года (×9,9), рост «+886 %» и forward P/E 1,12
    # на карточке. Встроенная перекрёстная проверка «прошлый год × темп» это НЕ
    # ловит: обе оценки растут от одной и той же перекошенной базы и сходятся.
    # Порог намеренно грубый (×3) и только при share_years < 2: по замеру всех 261
    # карточки блок показывается у 26, и под правило попадает ровно MTSS — второй по
    # величине множитель (ENPG ×4,7) опирается на ДВА года и остаётся.
    if (prof.get("share_years", 0) < 2 and prev_fy and prev_fy > 0
            and prof["mid"] > 3 * prev_fy):
        logger.info("run_rate %s: доля по одному году даёт ×%.1f к прошлому году — блок скрыт",
                    ticker, prof["mid"] / prev_fy)
        return {"status": "unreliable_seasonality", "year": year,
                "note": "сезонная доля известна по одному прошлому году, и он был "
                        "нетипичным — продолжение темпа дало бы кратный скачок"}
    if is_bank:
