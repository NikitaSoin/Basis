  ...
  return (
    <div className="icard">
      ...
      {/* ===== ВЕРДИКТ «ДОХОДНОСТЬ VS РИСК» — hero, без вкладки (методика bond_analys.md).
          2026-07-17: раньше это была вторая вкладка «Доходность vs риск» с дефолтом на
          «Обзор» — вердикт был на клик дальше. Владелец выбрал «вариант B»: вердикт сразу
...
const CompaniesView = ({ onSelectCompany, onSelectIndex, onSelectDriver, forceTab }) => {
...
const AgentAddendaStrip = ({ ticker }) => {
  ...
  // 🔴 Сигналы про ОТЧЁТНОСТЬ здесь не показываем (владелец, 2026-08-06). На
  // карточке ВТБ плашка сообщала «опубликована отчётность за 2кв2026, прибыль
  // 225,2 млрд», а блок «Вышел отчёт» ниже показывал разбор за 2025 — платформа
  // сама себе сообщала, что её же разбор устарел. Владелец: «это не должно
  // выводиться; если есть устаревание — оно должно автоматически правиться, а
  // не выводиться, что что-то новое появилось».
  // Первопричина (сортировка отчётов по периоду без года) исправлена на бэкенде;
  // здесь снимаем сам симптом: для отчётности на карточке есть собственный
  // блок разбора, и дублировать её баннером «есть новое» незачем.
  const latest = items.find((x) => x.kind === "signal_addendum"
    && !/отчётност|отчетност|МСФО|РСБУ|квартал|полугод/i.test(
         String((x.content || {}).headline || "")));
...
const CompanyCard = ({ company, onBack, initialTab, onTabChange }) => {
  ...
  const renderOverview = () => {
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // 🔴🔴 ВСЁ НИЖЕ ДО КОНЦА renderOverview — МЁРТВЫЙ КОД. НЕ ПРАВИТЬ ЗДЕСЬ. 🔴🔴
    // Достижимо только при `company.overview`, а /api/companies НЕ отдаёт это поле ни
    // для одной из 261 компании (проверено на живом API 2026-07-30) — выше стоит ранний
    // return, который срабатывает ВСЕГДА. Данные берутся из MOCK_COMPANIES, т.е. это
    // демо-заготовка. Живой «Обзор» — в ветке `if (!company.overview)` ВЫШЕ.
    // На эту ловушку уже наступали дважды: я принял здешний HERO за конкурирующее третье
    // число справедливой цены, а дизайн-агент чуть не переверстал недостижимый блок.
    // Тот же класс ошибки, что renderFinancials vs FinanceTab.jsx. Удаление блока —
    // отдельная задача (docs/status.md), делать её надо с проверкой «зазоров»: часть
    // хелперов отсюда может использоваться живыми ветками.
    // ═══════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className={NEO ? "cc-root tw-space-y-6" : "space-y-6"}>
      ...
      <ScrollRail minCount={3} minWidth={1280} deps={[tab, company && company.ticker]}
