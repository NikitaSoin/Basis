...
# живых данных: при радиусе села 3 км и окне «во весь зазор» каждый заявленный
# пункт добавлял 35–39 км² (3,2–3,5 тыс. км² сверх ISW на 90 пунктов) — село с
# округой столько не занимает. Теперь радиус села 1,5 км, окно вдоль фронта
# вокруг якоря — 0,4 зазора (+1 км), от 3 до 7 км: ~22 км² на пункт (2,0 тыс.
# км² сверх ISW на 90 пунктов, было 3,2–3,6). Карман между фронтом и пунктом по-прежнему закрывается, но полосой
# шириной с само продвижение, а не «на весь горизонт». У городов окно шире
# (_CAP_CITY_RATIO), чтобы закрыть карман у Константиновки.
_CAP_RATIO = 0.4
_CAP_MIN_KM = 3.0
_CAP_MAX_KM = 7.0
...
def _absorb_overrides(ru_mass, overrides: list[dict], source_mass=None, ukraine_boundary=None,
    ...
    for o in ordered:
        ...
        # окно по фронту — половина зазора (+1 км), но не меньше минимума и не больше
        # потолка; у города — свой охват. Окно строится вокруг ЯКОРЯ на фронте, а не
        # вокруг пункта, поэтому «заходить за линию» ему не нужно.
        r_km = min(max(_CAP_MIN_KM, _CAP_RATIO * gap_km + 1.0), max(_CAP_MAX_KM, city_reach))
        r_km = max(r_km, city_reach)
        ...
        # окно — прямоугольник 2R вокруг ЯКОРЯ (ближайшей точки фронта), clip_by_rect
        # в разы быстрее пересечения с кругом
        ax, ay = anchor.x, anchor.y
        facing_parts = [clip_by_rect(source_boundary, ax - r_lon, ay - r_deg, ax + r_lon, ay + r_deg)]
        if added is not None and not added.is_empty:
            facing_parts.append(clip_by_rect(added.boundary, ax - r_lon, ay - r_deg, ax + r_lon, ay + r_deg))
