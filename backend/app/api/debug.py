...
async function anLoad(days){
  ...
  // Сверка идёт СРАЗУ после Метрики и до наших цифр: иначе читатель успевает решить,
  // что одно из двух чисел неверно, и дальше листает с недоверием ко всему отчёту.
  const rec=d['сверка']||[];
  if(rec.length){
    h+='<h2>Где правда: наш лог против Метрики</h2>'
      +'<p class="sub">Числа расходятся не потому, что кто-то ошибается, а потому что '
      +'считают разное. В каждой строке — какой счётчик отвечает на этот вопрос.</p>'
      +anTable(rec,[['показатель','показатель'],['наш лог','наш_лог'],['Метрика','метрика'],
        ['кому верить','кому_верить'],['почему','почему']]);
  }
  // 🔴 Блок читал d['по_поведению'], которого в ответе НЕТ — карточки рисовались пустыми
  // (ключ называется 'отсев'). Отказ был молчаливым: пустая карточка выглядит как «нет
  // данных за период», а не как ошибка кода, поэтому и жил незамеченным.
  const bh=d['отсев']||{};
  h+='<h2>Наш лог: кто реально работал с платформой</h2>'
    ...
    +anCard(bh['отпечатков_всего'],'всего браузеров')+anCard(bh['отсеяно_визитов'],'визитов отсеяно')
    +'</div>'
    +'<p class="sub">'+(bh['правило']||'')+'</p>';
  h+='<h2>Сколько времени проводят (наш лог)</h2>'
    +'<p class="sub">Время — сумма реально прожитого на страницах визита, не расстояние '
    +'между первым и последним событием: по окну визита среднее завышалось в 4,4 раза '
    +'(человек мог отойти и вернуться). Столбец «по замеру» показывает, у скольких визитов '
    +'время измерено, а не восстановлено по окну — замеры идут с 06.09.2026.</p>'
    +anTable(d['время_на_платформе'],[['длительность визита','корзина'],['визитов','визитов','n'],
      ['людей','людей','n'],['в среднем, сек','средне_секунд','n'],
      ['из них по замеру','из_них_по_замеру','n']]);
...
def payment_refund(order_id: str = Query(..., description="номер заказа Basis"),
    ...
    """Вернуть платёж со стороны платформы (метод Cancel у банка).

    Обычный путь возврата — личный кабинет банка; эта ручка нужна, когда вернуть
    надо быстро и по номеру НАШЕГО заказа. Подписку снимает не она, а нотификация
    банка о возврате — так один и тот же путь работает независимо от того, откуда
    возврат запустили.

    🔴 Операция необратимая и с чужими деньгами, поэтому живёт под отладочным
    токеном и никогда — в открытых ручках."""
    from app.db.session import SessionLocal
    ...
    try:
        ...
        if p is None:
            return {"ok": False, "reason": f"заказ {order_id} не найден"}
        if not p.payment_id:
            return {"ok": False, "reason": "у заказа нет PaymentId — в банке платежа нет"}
        try:
            res = tb.cancel_payment(p.payment_id,
                                    int(amount_rub * 100) if amount_rub else None)
        except tb.AcquiringError as e:
            return {"ok": False, "reason": str(e), "error_code": e.code}
        return {"ok": True, "order_id": order_id, "payment_id": p.payment_id,
                "status": res.get("Status"),
                "original_amount": res.get("OriginalAmount"),
                "new_amount": res.get("NewAmount"),
                "note": "Подписку снимет нотификация банка о возврате."}
    finally:
        db.close()
...
# Заказ владельца 2026-08-26: «сделать в консоли, чтобы можно было посмотреть Метрику,
# Search Console (по дням/неделе/месяцу), распределение времени, сколько людей заходило
# несколько раз, какие страницы буквами („Рынок: акции“) и агрегированно по блокам».
#
# 🔴 Три источника, и они НЕ взаимозаменяемы — в этом весь смысл собрать их рядом:
#   наш лог      — что человек делал ВНУТРИ платформы (разделы, время, возвраты);
#   Метрика      — откуда пришёл и сколько среди визитов роботов (isRobot есть только в API);
#   Search Console — что показывает Google и по каким запросам.
...
vis AS (SELECT anon_id, vn, date(min(created_at)) AS d,
               ...
               -- 🔴 ВРЕМЯ НА ПЛАТФОРМЕ СЧИТАЕТСЯ ПО ПРОЖИТОМУ, А НЕ ПО ОКНУ ВИЗИТА.
               -- `sec` — это расстояние между первым и последним событием: человек открыл
               -- страницу, ушёл на 20 минут и вернулся в пределах получаса — окно даёт
               -- 20 минут, хотя читал он две. Замер на бою 11.09.2026: среднее окно 223 с
               -- против 51 с реально прожитых, разница в 4,4 раза — из-за неё наш отчёт
               -- показывал 4,3 минуты там, где Метрика показывала 1,3, и цифры выглядели
               -- взаимно противоречивыми. Берём сумму duration_ms по страницам визита;
               -- если её нет (события до 06.09.2026, когда замеров ещё не было), падаем
               -- обратно на окно — но такие визиты помечены отдельно, см. `по_замеру`.
               CASE WHEN COALESCE(sum(duration_ms), 0) > 0
                    THEN COALESCE(sum(duration_ms), 0) / 1000.0
                    ELSE EXTRACT(EPOCH FROM (max(created_at) - min(created_at))) END AS stay_sec,
               (COALESCE(sum(duration_ms), 0) > 0) AS по_замеру,
...
def analytics_data(days: int = Query(30, ge=1, le=365)):
    ...
    with SessionLocal() as db:
        ...
        rows = [dict(x) for x in q("""
            SELECT CASE WHEN stay_sec < 30  THEN 'меньше 30 секунд'
                        WHEN stay_sec < 60  THEN '30 секунд — 1 минута'
                        WHEN stay_sec < 180 THEN '1 — 3 минуты'
                        WHEN stay_sec < 600 THEN '3 — 10 минут'
                        ELSE 'больше 10 минут' END AS корзина,
                   count(*) AS визитов, count(DISTINCT anon_id) AS людей,
                   round(CAST(avg(stay_sec) AS numeric)) AS средне_секунд,
                   count(*) FILTER (WHERE по_замеру) AS из_них_по_замеру
            FROM hv GROUP BY 1
        """)]
        ...
        out["по_дням"] = [dict(x) for x in q("""
            SELECT d AS день, count(*) AS визитов, count(DISTINCT anon_id) AS людей,
                   round(CAST(avg(stay_sec)/60 AS numeric), 1) AS минут_на_визит
    ...
    out["сверка"] = _reconcile(out)
    return out


def _reconcile(out: dict) -> list[dict]:
    """Сверка нашего лога с Яндекс.Метрикой: одна строка — один показатель.

    🔴 ЗАЧЕМ. Владелец 11.09.2026: «по статистике от Метрики и нашей — по-моему, там куча
    противоречий, время, количество человек, вообще непонятно, где правда». Противоречий
    и правда много, но почти все они — не ошибка, а РАЗНЫЕ ОПРЕДЕЛЕНИЯ одного слова.
    Пока эти определения живут в головах, каждая новая сессия заново решает, какой цифре
    верить, и выбирает другую. Поэтому сверка не «объяснение задним числом», а часть
    отчёта: рядом с каждым расхождением написано, какой из двух счётчиков отвечает на
    какой вопрос, и какому верить именно в этом вопросе.
    """
    m = out.get("метрика") or {}
    o = out.get("отсев") or {}
    if not m.get("ok"):
        return []
    days = out.get("дней", 0)
    dni = [x for x in (out.get("по_дням") or []) if x.get("минут_на_визит") is not None]
    our_min = round(sum(x["минут_на_визит"] * x["визитов"] for x in dni)
                    / max(sum(x["визитов"] for x in dni), 1), 1) if dni else None
    qual = out.get("качество_данных") or {}
    return [
        {"показатель": "Сколько людей заходило",
         "наш_лог": o.get("людей"), "метрика": m.get("людей"),
         "кому_верить": "Метрике",
         "почему": "Метрика считает КАЖДЫЙ браузер, открывший любую страницу. Наш лог "
                   "считает только тех, кто задержался: визит признаётся человеческим, "
                   "если было действие или суммарно 15 секунд. Поэтому у нас всегда "
                   "меньше — это не потеря данных, а другой вопрос: «сколько людей "
                   "пришло» против «сколько людей работало»."},
        {"показатель": "Сколько визитов",
         "наш_лог": o.get("визитов_всего"), "метрика": m.get("визитов_всего"),
         "кому_верить": "нашему логу (сырое) и Метрике (очищенное)",
         "почему": f"У нас {o.get('визитов_всего')} сырых визитов, из них "
                   f"{o.get('визитов_людей')} признаны человеческими, "
                   f"{o.get('отсеяно_визитов')} отсеяно. Метрика отдаёт уже очищенное "
                   "число и своих отсеянных не показывает. Сравнивать надо "
                   "«визитов_людей» с числом Метрики, а не сырое с очищенным."},
        {"показатель": "Минут на визит",
         "наш_лог": our_min, "метрика": m.get("минут_на_визит"),
         "кому_верить": "нашему логу — с 06.09.2026",
         "почему": "Мы считаем сумму времени, реально прожитого на страницах визита. "
                   "Метрика считает по-своему — по расстоянию между первым и последним "
                   "событием, и последняя страница визита у неё вообще не учитывается. "
                   "🔴 До 11.09.2026 мы считали так же, как Метрика, по окну визита, и "
                   "получали завышение в 4,4 раза (223 секунды окна против 51 секунды "
                   f"прожитых). Замеры времени появились 06.09.2026: сейчас с замером "
                   f"{qual.get('с_замером_времени')} событий из {qual.get('событий')}, "
                   "для остальных время по-прежнему считается по окну."},
        {"показатель": "Глубина (страниц за визит)",
         "наш_лог": (round(out.get("просмотров_всего", 0) / o["визитов_людей"], 1)
                     if o.get("визитов_людей") else None),
         "метрика": m.get("глубина"),
         "кому_верить": "обоим, но это разные множества",
         "почему": "Наша глубина считается только по человеческим визитам, у Метрики — "
                   "по всем. Визит «открыл и сразу закрыл» тянет её среднее вниз, а из "
                   "нашего он исключён."},
        {"показатель": "Период",
         "наш_лог": f"{days} дней", "метрика": m.get("период"),
         "кому_верить": "—",
         "почему": "Наш лог считает от полуночи по дням, Метрика отдаёт свой диапазон "
                   "и обновляет последние сутки с задержкой. Разница в один день на "
                   "границе периода — нормальна и не является расхождением."},
    ]
