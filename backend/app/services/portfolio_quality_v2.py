...
def _value_score(rau_pct: float | None) -> int | None:
    """Балл запаса прочности по взвешенному потенциалу (в %). ДВУХЗОННАЯ шкала с
    опорной точкой 50 = «портфель куплен ровно по справедливой цене».

    🔴 Найдено 2026-07-31 (владелец: «портфель из Северстали, Ростелекома и Магнита —
    83 балла запаса прочности, это бред»). Была одна линейная шкала
    `_lin_score(rau, best=+25, worst=−90)`, и на ней НУЛЕВОЙ потенциал давал 78 баллов,
    а переплата в 20 % — 61 балл. То есть «запаса прочности нет» читалось как «запас
    прочности отличный». Растянули её ради различимости портфелей (по BFV медиана
    потенциала рынка −51 %, положительный лишь у 32 % бумаг), но различимость купили
    ценой смысла: балл перестал отвечать на свой собственный вопрос.

    Двухзонная шкала сохраняет и то, и другое:
      потенциал  0 %  → ровно 50 («куплен по справедливой цене, запаса нет»);
      потенциал +50 % → 100; +25 % → 75 (смысл якоря методики не изменился);
      потенциал −50 % → 25; −100 % → 0 — медианный по рынку портфель садится примерно
      на 25, то есть портфели по-прежнему РАЗЛИЧАЮТСЯ, но ниже середины, как и должно
      быть на рынке, который модель считает дорогим.
    🔴 Наклоны — продуктовое решение, ждут приёмки владельца (docs/status.md).
    """
    if rau_pct is None:
        return None
    if rau_pct >= 0:
        return round(50 + 50 * min(rau_pct / 50.0, 1.0))
    return round(50 * max(1.0 + rau_pct / 100.0, 0.0))
...
def _compute_v(equity: list[dict], fair_map: dict | None = None) -> dict | None:
    ...
    v_score = _value_score(portfolio_rau * 100)
    if v_score is None:
        return None
    coverage_pct = round(covered_w / total_w * 100)
    return {
        ...
        # 🔴 Вердикт привязан к САМОМУ потенциалу, а не к баллу (2026-07-31). Раньше
        # порогом был v_score >= 60, и портфель с ОТРИЦАТЕЛЬНЫМ потенциалом (−0.6 %)
        # получал 78 баллов и подпись «куплен с запасом прочности» — текст прямо
        # противоречил числу, стоявшему рядом с ним.
        "verdict": (
            "В среднем портфель куплен заметно дешевле модельной справедливой цены — запас прочности есть."
            if portfolio_rau >= 0.10 else
            "В среднем портфель куплен немного дешевле модельной справедливой цены — запас прочности небольшой."
            if portfolio_rau > 0.02 else
            "Портфель в среднем куплен примерно по справедливой цене — запаса прочности практически нет."
            if portfolio_rau >= -0.02 else
