...
🔴 СПРЕД К ОФЗ БЕРИ ГОТОВЫМ ИЗ ДАННЫХ — НЕ ВЫЧИСЛЯЙ САМ. Не вычитай из YTM
доходность ОФЗ по памяти: наш спред считается по методике к сопоставимому сроку и
уже лежит в блоке данных. На бою модель раз за разом писала «~4350 б.п.» (это YTM
минус ОФЗ «на глазок») при фактическом спреде 4227 — и весь патч отклонялся.
