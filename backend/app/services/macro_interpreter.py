...
def _snapshot_text(snapshot: dict) -> str:
    """Снапшот во вход — компактно и с ПОТОЛКОМ.

    🔴 Один шаг агента сжёг 590 тысяч токенов и упёрся в бюджет, не сделав ничего.
    Виноват вход: снапшот сериализовался с отступами и без ограничения размера. У
    одиночного вызова это сходило с рук — платили один раз; у агента КАЖДЫЙ шаг
    отправляет всю историю заново, и гигантский вход умножается на число шагов.
    Отступы убраны (полтора-два раза короче на ровном месте), размер ограничен.
    Обрезка — с явной пометкой: пусть модель знает, что видит не всё, а не считает,
    будто данных больше нет.
    """
    MAX = 90_000
    body = json.dumps(snapshot, ensure_ascii=False)
    if len(body) <= MAX:
        return body
    logger.warning("Интерпретатор: снапшот %d знаков — обрезан до %d", len(body), MAX)
    return (body[:MAX]
            + f"\n…[СНАПШОТ ОБРЕЗАН: {len(body)} знаков не помещаются целиком. "
              f"Если для вывода нужен показатель, которого не видно — скажи об "
              f"этом в data_flags, а не додумывай его значение]")
...
def generate(db: Session) -> MacroInterpretation:
    ...
    # 🔴 Мост «макро → сектор» тоже переехал на полку (id macro_sector): это такая
    # же методичка, и правило одно для всех — агент открывает нужный раздел сам.
    # В промпт он больше не вклеивается (был 30 тысяч знаков в каждом прогоне).
    # Оперативная выжимка драйверов — не методика, а ДАННЫЕ (что сейчас движет
    # секторами), поэтому она остаётся во входе.
    from app.services.macro_sector_playbook import drivers_digest
    drivers = drivers_digest()
    # 🔴 МЕТОДИЧКИ УБРАНЫ ИЗ ПРОМПТА (владелец, 2026-08-20). Здесь их было больше
    # всего: методичка макро (57 тыс. знаков) плюс ядро «институты → макро» плюс
    # мост к секторам (30 тыс.) — системная часть доходила до 121 тысячи знаков,
    # около 40 тысяч токенов ДО того, как туда лягут снапшот и досье. Из-за этого
    # приходилось резать методику ПО РАЗМЕРУ, то есть терять содержание.
    # Теперь остаётся контракт с витриной, а методики агент открывает сам —
    # включая мост «макро → сектор», он тоже лежит на полке.
    system = _OUTPUT_SPEC
    ...
    def _ask(extra: str = "") -> dict:
        ...
        out = analyst.run(
            ...
            shelf_docs=["macro", "inst_macro", "macro_sector", "geo_macro",
                        "inst_env", "geo_inst", "inst_geo"],
