...
# ~10.8к строк). 30 дней (владелец, 2026-07-22) = окно чтения агента/корп-новостей
# (21-30д) — Лента остаётся display-лентой недавнего. Важное НЕ теряется: high/medium-
# с-тикерами уже осели в ПОСТОЯННОЙ летописи (chronicle), она агентам и служит памятью.
_LENTA_KEEP_DAYS = 30
...
_FILTER_SYS = (
    ...
    "importance — ШКАЛА ПЕРЕКАЛИБРОВАНА (владелец 2026-08-02: «важное — это прям важное»):\n"
    "• high — ЭКСТРАОРДИНАРНОЕ событие уровня всего рынка или системного эмитента, "
    "случается редко (единицы в неделю, НЕ десятки в день). Примеры-эталоны: ЦБ изменил "
    "ключевую ставку; началась/резко эскалировала война; закрыт/открыт Ормузский пролив "
    "или другая критическая артерия; повышены налоги для рынка/сектора; Сбер или другой "
    "системный эмитент отменил/резко изменил дивиденды; нефть/рубль рухнули или взлетели "
    "на двузначный процент; санкции на НКЦ/биржу/системную компанию; дефолт крупного "
    "эмитента. Если сомневаешься между high и medium — это medium.\n"
    "• medium — ЗНАЧИМОЕ, но ординарное: отчётности и дивиденды по графику, M&A, "
    "гайденсы, отраслевые решения, макро-статистика в рамках ожиданий, санкционная "
    "рутина. Это основной уровень ленты.\n"
    "• low (обычно keep=false) — слабый/неясный эффект.\n"
...
_SUMMARY_SYS = (
    "По каждой новости верни: (1) summary — пересказ с КОНКРЕТНОЙ ФАКТУРОЙ из "
    ...
    "ГЛУБИНА summary ЗАВИСИТ ОТ ВАЖНОСТИ (владелец 2026-08-02: где важны детали — "
    "подробнее): importance=\"high\" — ПОДРОБНЫЙ пересказ 4-6 предложений: вся ключевая "
    "фактура, предыстория/сравнение с прошлым значением ЕСЛИ есть в тексте, реакция рынка "
    "ЕСЛИ есть в тексте; таким новостям передан ПОЛНЫЙ ТЕКСТ статьи — используй его "
    "фактуру целиком, не ограничивайся первым абзацем. importance=\"medium\" — 2-3 "
    "предложения, как обычная лента. low — 1-2 предложения.\n"
...
_EXTRACT_SYS = (
    ...
    "неделя по 9 июня → 2026-06-09). Формат YYYY-MM-DD. as_of ВСЕГДА в прошлом или сегодня — "
    "период уже ЗАКОНЧИЛСЯ и число по нему уже ИЗВЕСТНО фактически. Если новость говорит "
    "«ожидается», «прогнозируется», «по итогам года составит», «планируется» — это ПРОГНОЗ на "
    "ещё не наступивший период, НЕ извлекай его вообще (прогнозы сюда не идут ни при каких "
    "условиях, даже с is_preliminary=true — это поле для официального числа, которое ещё уточнят, "
    "а не для чужого ожидания на будущее).\n"
    ...
    "чем записать неверно). Бери только то, что прямо в тексте. В ЧАСТНОСТИ: показатели "
    "«инфляция»/«недельная инфляция» — это ОБЩИЙ (агрегатный) индекс потребительских цен по всей "
    "корзине товаров и услуг, публикуемый Росстатом/ЦБ. НЕ путай его с изменением цены ОТДЕЛЬНОГО "
    "товара или категории (сахар, гречка, бензин, овощи и т.п.) — новость «цены на сахар выросли "
    "на 2,6% за неделю» НЕ является значением показателя inflation_weekly, даже если рядом в той "
    "же новости упомянута настоящая общая недельная инфляция — тогда извлекай именно её отдельным "
    "числом, а не число по конкретному товару.\n"
...
def summarize(reps: list[dict], batch: int = 15) -> dict:
    ...
    от него зависят глубина summary и impact (см. _SUMMARY_SYS).

    Для high (после рекалибровки — единицы в неделю) дотягивается ПОЛНЫЙ текст
    статьи по source_url: RSS-анонс в 2-3 предложения физически не даёт фактуры
    для подробного пересказа, а правило доверия запрещает догенерацию — без
    полного текста «подробнее» невозможно ЧЕСТНО. Фетч дёшев (high редкие);
    не скачалось — честная деградация на анонс."""
    ...
    for ch in _chunks(reps, batch):
        ...
        for r in ch:
            text = r["announce"]
            if r.get("importance") == "high" and r.get("url"):
                full = _fetch_article_text(r["url"], limit=9000)
                if full and len(full) > len(text) + 200:
                    text = full
            items.append({"id": r["id"], "title": r["title"], "text": text,
