...
def _valuation(fin: dict, model: dict, shares, unit_mult: float) -> tuple[dict | None, list[str]]:
    ...
    # 🔴 Цену карточки СЮДА НЕ ПИШЕМ. Здесь стоял card_fair_price_rub из
    # financials.json → fair_value_range.base, и фронт печатал его как «цену
    # карточки»: у МТС 178 ₽, тогда как карточка живьём показывала 68 ₽ (BFV).
    # Справедливая цена карточки считается ЖИВЬЁМ и на сборке неизвестна —
    # сверку с ней делает фронт по загруженному bfv. Отсюда идёт только своё
    # число модели, а показывать его или нет — решают уже там.
    # fair_value_range остаётся опорой ТОЛЬКО для порога публикации сверки:
    # он даёт порядок величины, чтобы не отдавать заведомо абсурдный расчёт.
    ...
    block["cross_check"] = {
        ...
        # расхождение против ЖИВОЙ цены карточки считает фронт; здесь — только
        # против опорного диапазона, как след порога публикации
        "divergence_from_range_pct": round(diff, 1),
        "note": ("Независимая сверка: прогнозная прибыль модели × исторический мультипликатор. "
                 "Показывается, только если сходится с живой оценкой карточки — "
                 "это подтверждение, а не второе мнение."),
    }
    return block, []
