...
_TIMEOUT = 25
# 🔴 Потолок на документ. Ставили 60k знаков («окно 1M, места полно»), и упёрлись НЕ в
# окно модели, а в таймаут прокси Timeweb: вход раздулся до 490k токенов, генерация не
# уложилась и ручное обновление отдало 502 (2026-08-01). Ограничение оказалось не в
# цене токенов и не в контексте, а во ВРЕМЕНИ ответа.
# 14k знаков ≈ 6k токенов — это 4-5 страниц: у записок ЦБ/ЦМАКП туда попадают резюме,
# выводы и ключевая аргументация, то есть та «суть», которой не было в нашей выжимке.
_MAX_CHARS = 14_000
