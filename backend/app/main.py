...
async def _macro_state_job():
    ...
    были только выпуски-тексты. Стоит ПОСЛЕ гео-барометра (21:50) — его сценарии
    и шоки входят в состояние ребром «гео → макро». Fail-closed внутри rebuild()."""
    def _run():
        from app.db.session import SessionLocal
        ...
        try:
            row = rebuild(db)
            if row is None:
                return "нет индикаторов — состояние не трогали"
            return {"id": row.id, "status": row.status, "gate_notes": row.gate_notes}
        finally:
            db.close()
    try:
        res = await asyncio.get_event_loop().run_in_executor(None, _run)
        logger.info("Ежедневная пересборка макро-состояния: %s", res)
    except Exception as e:
...
async def _probe_questions_job():
    ...
    экзаменатора, счёт в реестре качества (pipeline=probe). Суббота утром, по свежим
    пятничным сводкам."""
    def _run():
        from app.db.session import SessionLocal
    ...
    try:
        row = await asyncio.get_event_loop().run_in_executor(None, _run)
        logger.info("Контрольные вопросы: версия #%s, %s", getattr(row, "id", None),
                    ((getattr(row, "payload", None) or {}).get("summary")))
    except Exception as e:
        logger.exception("Ошибка контрольных вопросов: %s", e)
