...
def audit(card, ticker):
    ...
    for name, vals in named.items():
        ...
        for i in range(1, n):
            ...
            if not a or not b:
                continue
            # 🔴 Обе величины должны быть КРУПНЫМИ. Скачок «714 → 1» — это почти всегда
            # настоящая мелкая величина, а не съехавший масштаб: у Юнипро так честно
            # обнулялись депозиты, у ТНС энерго — долгосрочный долг. Настоящая же ошибка
            # единиц (валовые кредиты Авангарда) выглядит иначе: КРУПНОЕ число против
            # КРУПНОГО, отличающиеся ровно на три порядка. А случаи, где мелкое значение
            # действительно невозможно, ловит проверка «деталь больше содержащего целого».
