...
_MAX_FIND_LEN = 600     # якорь find: раньше 200 «≤ предложение», но интерпретация
...
# 2026-08-08: поднято 450 → 600 по разобранному случаю BANE/markets — отклонён
# find(497), а это был ОДИН размеченный абзац «**Где цена сейчас (факт, июль
# 2026).** Urals торговался в диапазоне...», который целиком и должен был стать
# августовским. Предел режет не переписывание, а законную правку блока. 600 —
# по-прежнему «не больше одного абзаца»: попытка заменить раздел (767 знаков,
...
_FACT_ADD_SYS = """Ты — редактор аналитической карточки платформы Basis (не брокер,
без «купить/продать» и прогнозов). Тебе дан ТЕКСТ разбора вкладки и ПОДТВЕРЖДЁННОЕ
СОБЫТИЕ, которого в этом тексте ЕЩЁ НЕТ.

Задача — ВПИСАТЬ событие в разбор. Это не факт-чек: искать в тексте старое число не
нужно, его там нет. Нужно найти абзац, к которому событие относится по смыслу, и
вернуть правку, где этот абзац заменён на тот же абзац ПЛЮС одно-два предложения о
событии — в том же стиле и с той же строгостью, что и остальной текст.

🔴 Правила вписывания:
• пиши, ЧТО произошло, КОГДА и ЧТО это значит для этой стороны бизнеса — одной мыслью,
  без пересказа новости и без эмоций;
• число из события можно ставить только если оно есть в переданных данных;
• «сообщение СМИ» подавай как сообщение («по данным <издание>»), «официальное
  раскрытие» — как факт;
• без фамилий должностных лиц: только должности;
• не дублируй: если событие в тексте уже описано другими словами — confirmed=false;
• абзац в replace возвращай ЦЕЛИКОМ, вместе с исходным текстом, а не одну новую фразу.

Финальный ответ строго JSON того же формата, что и у факт-правок."""


_FACT_SYS = """Ты — редактор-факт-чекер платформы Basis (не брокер, без «купить/
продать» и прогнозов). Тебе дан ТЕКСТ разбора вкладки и СИГНАЛ-СОБЫТИЕ. Задача: если
в тексте есть УСТАРЕВШИЙ ФАКТ (число, дата, уровень, показатель), который сигнал
делает неверным — верни ТОЧЕЧНЫЕ правки find/replace, меняющие ТОЛЬКО устаревший
факт. НЕ переписывай текст, НЕ добавляй интерпретацию, НЕ меняй смысл предложения
...
• «сообщение СМИ» — НЕЛЬЗЯ подавать как установленный факт. Либо припиши источник
  прямо в тексте («по сообщениям СМИ», «по данным Интерфакса»), либо не трогай
  фрагмент вовсе. certainty ставь "оценка". Особенно это касается смены
  руководства, сделок и исков: сообщение о переговорах — не состоявшаяся сделка.
  Разница между «компания продала актив» и «СМИ сообщили о переговорах о продаже» —
  это и есть разница между аналитикой и слухом.
...
def _apply_and_gate(prose: str, result: dict, signal_text: str,
    ...
    # ОКРУГЛЕНИЯ заземлённых чисел. Проза намеренно пишет «YTM ~53%», а не «53,07%»:
    # приблизительность здесь — часть жанра. Модель честно правит на «~57%» от
    # заземлённого 57.07 — и гейт резал это как ungrounded_numbers:['57'] (бой
    # 2026-08-08, ЦР БО-03 и разбор Brent). Разрешаем ровно округления того, что
    # уже заземлено: выдумать новое число этим нельзя, а живую правку больше не режет.
    for n in list(allowed_nums):
        try:
            v = float(n.replace(",", "."))
        except ValueError:
            continue
        allowed_nums |= {f"{round(v):g}", f"{round(v, 1):g}", f"{round(v, 2):g}",
                         f"{int(v):g}"}
    patched = prose
    for i, e in enumerate(edits):
        ...
        max_grow = 500 if kind in ("interpretation", "fact_add") else 200
...
def run_for_signal(db: Session, signal: CompanySignal, kind: str = "fact") -> CardProseOverlay | None:
    ...
    def _tb(prose: str, verification: dict | None = None) -> str:
        ...
        return (
            ...
            f"дата {signal.published_at}, ДОВЕРИЕ: "
    ...
    # 🔴 Находка активной ревизии — это НОВЫЙ факт, которого в разборе нет вовсе, и
    # обычный факт-чек его честно отвергает («такого в тексте не нашёл»). Именно так
    # ушла в отказ первая же настоящая находка (SBER: покупка 41,9% «Элемента» за
    # 27 млрд, подтверждённая первоисточником). Для ревизии — режим ДОПОЛНЕНИЯ.
    from_revision = (signal.source_key or "") == "revision_scout"
    return _run_patch(
        db, ticker, tab,
        sys=(_FACT_ADD_SYS if from_revision else _FACT_SYS) + _JSON_ONLY,
        task_builder=lambda prose: _tb(prose, check),
        grounding_text=grounding, kind="fact_add" if from_revision else "fact",
...
def _fact_queue(db: Session) -> list[CompanySignal]:
    ...
    # ТРИДЦАТЬ. По «Корпоративному управлению» — 1806 сигналов важности high и НИ
    # ОДНОГО в карточках: у управленческих новостей первоисточник почти всегда СМИ,
    # а не раскрытие эмитента. Владелец (2026-08-09): «карточки питаются не
    # ограниченным потоком, а всем, что идёт на платформу».
    # Поток открыт, но планка не снижена: сигнал из СМИ идёт с ЯВНОЙ пометкой
    # доверия, и патчер обязан приписать источник, а не выдать сообщение за факт
    # (см. _FACT_SYS). Гейт заземления чисел работает как прежде.
...
🔴 find КОПИРУЙ ДОСЛОВНО, ВМЕСТЕ С РАЗМЕТКОЙ. Звёздочки жирного шрифта, дефис
маркера списка, скобки — всё как в тексте. На бою правка потерялась именно так:
в прозе «- **Bear (30%):** Urals у нижней границы», а модель отдала find «Bear
(30%): Urals у нижней границы» без звёздочек — совпадения нет, и вся правка
отброшена. Проще всего копировать середину предложения БЕЗ разметки по краям.

🔴 ФОРМА ПРАВКИ: find — КОРОТКИЙ якорь, одна фраза или одно предложение (жёсткий
предел ~550 знаков, длиннее отбраковывается целиком). Не захватывай абзац целиком
и не переписывай раздел: одно расхождение — одна короткая правка. Если устарел САМ
заголовок — правь его отдельной правкой.
"""
...
# 🔴 Геополитика карточки — НЕ СИНОНИМ СВО (владелец, 2026-08-10: «в карточках по
# геополитике по-моему идёт только СВО, а должно поступать от всех очагов»).
# В контекст мы и так подаём все три очага, но модель по умолчанию тянется к самому
# «громкому». Здесь это названо прямо: очаг выбирается по тому, через что деньги
# компании реально ходят, а не по величине конфликта.
_GEO_ENV_SYS = _ENV_INTERP_SYS + """

🔴 УЧИТЫВАЙ ВСЕ ОЧАГИ, А НЕ ТОЛЬКО УКРАИНСКИЙ. В переданном состоянии есть три:
конфликт на Украине, Ближний Восток, Азиатско-Тихоокеанский регион. Для КАЖДОЙ
компании релевантен свой набор:
• экспортёр нефти — Ближний Восток (премия в цене, Ормузский пролив) не меньше, чем
  санкционный режим;
• металлург и производитель удобрений — спрос Китая и торговые барьеры (АТР);
• технологическая компания — доступ к оборудованию и цепочкам поставок (АТР);
• перевозчик и порт — маршруты и страхование по всем трём направлениям.
Если очаг для этой компании ничего не меняет — не пиши о нём вовсе. Но и не своди
геополитику к украинскому конфликту по умолчанию: это разные каналы до денег.

🔴 БАЛЛ ОЧАГА — ЭТО ВЛИЯНИЕ НА РОССИЙСКИЙ РЫНОК, а не острота конфликта. Высокая
напряжённость в АТР при низком балле — нормально: значит, до нашего рынка пока
доходит немного."""
...
def run_geo_env_interp(db: Session, batch: int = _ENV_BATCH,
    ...
    return _run_env_interp(db, tab="geo", grounding=grounding, queue=queue,
                           kind="interpretation", sys_prompt=_GEO_ENV_SYS)
...
🔴 СЦЕНАРНЫЕ БЛОКИ: НЕ ПЕРЕСЧИТЫВАЙ, НО И НЕ ОСТАВЛЯЙ МОЛЧА. Если в тексте есть
сценарии (база/бык/медведь) с ценовыми уровнями, сверь их с текущей ценой. Реальный
случай: цену Urals обновили до $75, а рядом остались «медведь: у нижней границы
$40–47» и «бык: цена $62–65» — то есть ОПТИМИСТИЧНЫЙ сценарий оказался НИЖЕ факта,
и весь блок читается как ошибка. Новые уровни сам НЕ ПРИДУМЫВАЙ — их считает
аналитик по методике. Вместо этого добавь к блоку короткую честную оговорку без
единого нового числа, например: «Сценарии строились при существенно более низкой
цене и требуют пересмотра». Одна такая правка на весь блок.
"""
...
🔴 ПРОВЕРЬ СОСЕДНЕЕ ПРЕДЛОЖЕНИЕ. Обновив число, посмотри, не противоречит ли ему
фраза рядом про диапазон или направление движения. Реальный пример: цену золота
поправили на $4 073, а следом осталось «скорректировался в диапазон $4 300–4 400» —
текст стал спорить сам с собой. Такую фразу правь ОТДЕЛЬНОЙ короткой правкой (или,
если свежих данных для неё нет, убери только спорное утверждение о диапазоне,
оставив остальное предложение).

🔴 ЧИСЛА ПЕРЕНОСИ ДОСЛОВНО. Разрешено только отбросить дробную часть (57.07 → «~57%»)
или оставить её целиком («57,07%»). ЗАПРЕЩЕНО округлять до «красивого» числа: из
спреда 4227 нельзя сделать «~4350» — это выдуманное значение, и вся правка целиком
будет отклонена. Одна выдумка отменяет ВЕСЬ патч, включая верные правки рядом:
на карточке не должно оказаться двух разных значений одного показателя.
...
🔴 ФОРМА ПРАВКИ: find — КОРОТКИЙ якорь (одна фраза или одно предложение, не длиннее
~550 знаков). Не захватывай в один find абзац целиком вместе с заголовком — такая
правка отбраковывается как попытка переписать текст. Одно расхождение — одна правка.
При этом если УСТАРЕЛ САМ ЗАГОЛОВОК (например «## Текущий контекст (июнь 2026)», а
данные уже за август) — правь его ОТДЕЛЬНОЙ короткой правкой: заголовок с прошлым
месяцем над свежими числами читается как ошибка."""
...
🔴 СПРЕД К ОФЗ БЕРИ ГОТОВЫМ ИЗ ДАННЫХ — НЕ ВЫЧИСЛЯЙ САМ и НЕ ОКРУГЛЯЙ. Наш спред
считается по методике к сопоставимому сроку и уже лежит в блоке данных. На бою
модель раз за разом писала «~4350 б.п.» при фактическом спреде 4352 — округление
до «красивого» числа, и весь патч из-за него отклонялся 25 раз подряд. На
четырёхзначном спреде такое округление не «приблизительность», а потеря точности.
...
# =============================================================================
# ВКЛАДКА «КОРПОРАТИВНОЕ УПРАВЛЕНИЕ»: доводка под институциональную среду
#
# Владелец (2026-08-08): «инфраструктура обновления карточек — рынки и
# корпуправление?». По рынкам контур был, по управлению — нет: вкладку правил
# только сигнал ПРО КОМПАНИЮ (дивиденд объявлен, сменился совет, иск). Среда же
# меняется без таких новостей: замеры направлений показывают защиту собственности
# 2.0/5 и ухудшение, суды 3.0/5 — а в прозе карточки риск описан по состоянию на
# момент написания.
#
# 🔴 ГРАНИЦА УЖЕ, ЧЕМ У ДРУГИХ ВКЛАДОК. Управление — это в основном ФАКТЫ о
# компании: кто владеет, сколько free-float, что в дивполитике, кто в совете.
# Среда не меняет ни один из них. Она меняет только ОЦЕНКУ РИСКА: насколько
# вероятно изъятие, есть ли шанс отстоять права в суде, чем грозит рост госдоли.
# Поэтому патчер правит утверждения о риске и НЕ ТРОГАЕТ фактическую часть.
#
# 🔴 ДИВИДЕНДЫ СЮДА НЕ ЗАВОДИМ. Казалось бы, естественный вход: «платит стабильно»
# против факта выплат. Но таблица dividends на бою пуста за последние 200 дней
# (последняя запись 14.10.2025) — источник MOEX ISS перестал отдавать свежие
# выплаты. Патчить дивполитику от протухшей таблицы — значит вписать в карточку
# «не платит с 2025» там, где выплата могла быть. Пока источник не починен,
# дивиденды остаются за сигнальным контуром (объявление выплаты — это событие,
# оно приходит новостью).
# =============================================================================
_GOV_ENV_SYS = """Ты — аналитик-редактор платформы Basis (не брокер, без
«купить/продать» и целевых цен). Даны СОСТОЯНИЕ ИНСТИТУЦИОНАЛЬНОЙ СРЕДЫ (замеры
Обозревателя по направлениям) и ТЕКСТ вкладки «Корпоративное управление» карточки.

Правь ТОЛЬКО утверждения о РИСКЕ, которые разошлись со средой: вероятность изъятия
или пересмотра приватизации, шансы миноритария защититься в суде, давление
государства на компанию, последствия роста госдоли и монополизации.

🔴 НЕ ТРОГАЙ ФАКТИЧЕСКУЮ ЧАСТЬ — среда её не меняет:
• кто владеет компанией, размер долей, free-float, структура холдинга;
• дивидендная политика и история выплат (свежих данных о выплатах у тебя НЕТ —
  любые правки про дивиденды запрещены);
• состав совета директоров, менеджмент, их биографии;
• конкретные события: иски, сделки, оферты, даты собраний.
Никаких имён должностных лиц: пиши нейтрально по должности.

🔴 ЕСЛИ СРЕДА НЕ МЕНЯЕТ НИ ОДНОГО УТВЕРЖДЕНИЯ В ТЕКСТЕ — верни пустой список
правок. Пустой ответ здесь нормален и лучше натянутой правки: у большинства
компаний вкладка описывает их собственную структуру, на которую замеры среды
никак не влияют."""


def _gov_env_grounding(db: Session) -> str:
    """Институциональная среда в разрезе, значимом для управления."""
    from app.services.institutions_domains import for_agents
    data = for_agents(db)
    doms = data.get("domains") or {}
    if not doms:
        return ""
    # Только направления, которые реально меняют оценку риска управления.
    # Остальные (например инфляционный режим) к правам акционера отношения не имеют.
    titles = {
        "property": "защита частной собственности (риск изъятия, пересмотра приватизации)",
        "courts": "суды (шансы отстоять права, в том числе миноритарию)",
        "state_share": "доля государства в экономике",
        "monopoly": "монополизация рынков",
        "competition": "состояние конкуренции",
        "business_state": "конфликты бизнеса и государства",
        "lobbying": "возможность отраслевых интересов влиять на решения",
    }
    parts = [f"ИНСТИТУЦИОНАЛЬНАЯ СРЕДА (замер на {data.get('as_of')}), шкала 1-5, "
             f"где 1 — среда враждебна собственнику, 3 — нейтральна, 5 — защищает:"]
    for key, title in titles.items():
        d = doms.get(key)
        if not d:
            continue
        parts.append(f"  {title}: {d.get('score')} из 5, динамика — {d.get('direction')}")
    for ch in (data.get("changes") or [])[:4]:
        parts.append(f"  СДВИГ: {ch.get('label')} {ch.get('from')} → {ch.get('to')}"
                     + (f" ({ch.get('why')[:160]})" if ch.get("why") else ""))
    parts.append(f"Сегодня: {datetime.now(timezone.utc).date().isoformat()}")
    return "\n".join(parts)


def run_gov_env_interp(db: Session, batch: int = _ENV_BATCH,
                       only_ticker: str | None = None) -> dict:
    """Доводка вкладок «Корпоративное управление» под институциональную среду."""
    grounding = _gov_env_grounding(db)
    if not grounding:
        return {"error": "нет замеров институциональной среды"}
    if only_ticker:
        queue = [only_ticker.upper()]
    else:
        # Кандидаты — те, у кого вкладка вообще есть; очередь двигает cooldown.
        candidates = sorted(
            p.name for p in COMPANIES_DIR.iterdir()
            if p.is_dir() and (p / "governance_summary.md").exists())
        queue = _env_queue(db, "governance", candidates, batch)
    return _run_env_interp(db, tab="governance", grounding=grounding, queue=queue,
                           kind="interpretation", sys_prompt=_GOV_ENV_SYS)
