...
_FILTER_SYS = (
    ...
    "ОСТАВЛЯЙ: решения ЦБ по ставке, инфляцию и макростатистику (ВВП, промпроизводство); "
    "санкции и крупные регуляторные изменения; крупные корпоративные события (отчётности, "
    "M&A, дивиденды, байбэки, смена ключевого менеджмента крупных эмитентов, крупные сделки); "
    "движения по нефти/газу/ключевым сырьевым товарам/курсу рубля; крупные мировые "
    "экономические события (ФРС, мировые рынки, спрос), если влияют на РФ-рынок; геополитику "
    "с ПРЯМЫМ понятным влиянием на рынок.\n"
...
_SUMMARY_SYS = (
    "По каждой новости верни: (1) summary — выжимка 1-2 предложения, ТОЛЬКО факты из текста "
    "источника; (2) impact — 1-2 предложения «на что влияет»: какой рынок/сектор/бумаги и "
    "через какой механизм. ЖЁСТКОЕ ПРАВИЛО: используй ТОЛЬКО информацию из переданного текста, "
    "не добавляй цифры/имена/даты/прогнозы, которых нет в источнике; не хватает данных — пиши "
    "обобщённо, НЕ ВЫДУМЫВАЙ; не давай торговых рекомендаций. "
