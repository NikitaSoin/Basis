...
#
# 🔴 Зарплаты Росстата и квартальный ВВП ОТСЮДА УБРАНЫ 2026-08-19: у Росстата нашлась xlsx-таблица
# (см. macro_rosstat_wages_sync) — сначала файл источника, потом веб-поиск.
...
_TARGETS = {
    # 🔴 ВВП остаётся здесь: у Росстата есть квартальная таблица, но она разложена на
    # полтора десятка листов (текущие цены и три базы постоянных, каждый лист за свой
    # отрезок лет) — разбор трижды выбрал не тот лист и дал неверные уровни. Пока не
    # разобрана надёжно, честнее добирать последнее значение поиском, чем публиковать
    # ряд, который выглядит полным и врёт.
