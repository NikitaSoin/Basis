    ...
    try:
        ...
        if kind == "paid":
            ...
            body = (
                ...
                # Человек получает несколько писем от разных отправителей и не обязан
                # понимать, кто из них кто. Без этой строки «а где чек?» идёт к нам,
                # хотя чек формирует и отправляет касса, а не платформа.
                f"Фискальный чек придёт отдельным письмом от онлайн-кассы.\n\n"
