...
def is_near_offer_artifact(bond: dict) -> bool:
    """YTM-артефакт близкого «короткого хвоста»: до ближайшего события (пут-оферта,
    call-оферта ИЛИ само погашение) считаные дни/месяцы, цена у номинала, а YTM
    технически раздут коротким горизонтом (скидка к номиналу размазана на годовую
    шкалу). Это НЕ премия за риск и НЕ дистресс (дистресс = цена сильно ниже номинала).
    Дискриминатор — цена ≥ ~93%. Покрывает near-offer / near-call / near-maturity."""
    # ближайшее из событий: оферта (пут/колл) или погашение
    days_list = [d for d in (_days_to(bond.get("offer_date")),
                             _days_to(bond.get("maturity_date"))) if d is not None]
    d_near = min(days_list) if days_list else None
    ytm, price = bond.get("ytm"), bond.get("last_price")
    return (d_near is not None and 0 <= d_near <= 120 and ytm is not None and ytm > 35
...
def yield_vs_risk(bond: dict, group_medians: dict) -> dict | None:
    ...
    if is_near_offer_artifact(bond):
        ...
        d_mat = _days_to(bond.get("maturity_date"))
        # что ближе — оферта или погашение
        near_is_offer = d_off is not None and (d_mat is None or d_off <= d_mat)
        if near_is_offer:
            ev, d_near, tail = "пут-оферты", d_off, ("Корректно оценивать доходность можно только после "
                "оферты, когда эмитент назначит новый купон.")
        else:
            ev, d_near, tail = "погашения", d_mat, ("Реальная отдача здесь — это сам возврат тела к номиналу "
                "плюс оставшиеся 1–2 купона, а не «годовая» цифра.")
        return {"light": "gray", "near_offer": True, "simple_verdict": True,
                "label": f"Доходность искажена близким {'погашением' if not near_is_offer else 'офертой'}",
                "verdict": (f"До {ev} ~{d_near} дн., бумага у номинала — поэтому «доходность к погашению» "
                            f"({bond.get('ytm'):.0f}%) технически раздута коротким сроком и НЕ отражает ни риска, "
                            f"ни реальной отдачи. Это не дистресс. {tail} Кредитное качество — см. рейтинг и разбор ниже."),
                "certainty": "оценка (артефакт короткого хвоста)"}
