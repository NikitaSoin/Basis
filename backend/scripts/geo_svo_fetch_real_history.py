...
Отставание архива — норма, а не сбой. ISW публикует помесячные таймлапсы с
задержкой (на 11.09.2026 последний доступный — май 2026, а MONTH_SOURCES
руками заведён до него же), и раньше это тихо ломало ряд «км²/мес»: месяцы
между концом архива и сегодня из ряда ВЫПАДАЛИ, а их движение приписывалось
текущему месяцу (владелец, 2026-09-11: «данных за август нет, и что-то
посчитано за сентябрь»). Теперь дыры закрывает мост из СВОИХ дневных
снапшотов чистой ISW-площади (geo_frontline_snapshot.isw_area_km2, см.
geo_svo_capture_isochrone._own_isw_areas_by_month), а месяц, которого нет ни
там ни там, честно помечен «данных нет» и НЕ отдаёт свою дельту соседу.
Прогонять этот скрипт по-прежнему стоит, когда ISW выкладывает новые
таймлапсы: сначала пополни MONTH_SOURCES их именами (список сервисов —
GET {BASE}?f=json), архивный срез точнее нашего снапшота.
