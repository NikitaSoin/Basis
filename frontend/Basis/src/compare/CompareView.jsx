...
const CMP_YEARLY_METRICS = [
  { key: "revenue", label: "Выручка", unit: " млрд ₽", source: "income" },
  { key: "net_profit", label: "Чистая прибыль", unit: " млрд ₽", source: "income" },
...
export default function CompareView({ onOpenCompany }) {
  ...
  const yearlyMetricDef = CMP_YEARLY_METRICS.find((m) => m.key === yearlyMetric);
  const yearlySeries = useMemo(() => {
    const isIncome = yearlyMetricDef?.source === "income";
    ...
    return items
      .map((it, i) => {
        const fin = finData[it.ticker];
        // Абсолютная выручка/чистая прибыль — из income_statement (обычные
        // компании) или bank_pnl (банки, те же имена полей); metrics_timeseries
        // такого не считает — там только ставки/маржи/мультипликаторы.
        const arr = isIncome
          ? (fin?.income_statement?.[yearlyMetric] ?? fin?.bank_pnl?.[yearlyMetric])
          : fin?.metrics_timeseries?.[yearlyMetric];
        const fy = fin?.meta?.fiscal_years;
        if (!arr || !fy) return null;
        const unitDiv = isIncome ? 1000 : 1; // meta.unit="млн" → млрд ₽ для графика
        const byYear = {};
        fy.forEach((y, idx) => { byYear[y] = arr[idx] != null ? arr[idx] / unitDiv : null; });
        return {
          name: it.name,
      ...
      .filter(Boolean);
  }, [items, finData, yearlyMetric, yearlyMetricDef]);
          ...
          <div ref={yearlyCardRef} className="cmp-panel" style={{ marginTop: 20 }}>
            ...
            {finLoading ? (
              ...
              <>
                ...
                {yearlyChartType === "bar" ? (
                  <CompareBarChart series={yearlySeries} viewW={1000} viewH={260} unit={yearlyMetricDef?.unit || ""} />
                ) : (
                  <ObsLineChart series={yearlySeries} viewW={1000} viewH={260} unit={yearlyMetricDef?.unit || ""} />
                )}
              </>
            ) : (
