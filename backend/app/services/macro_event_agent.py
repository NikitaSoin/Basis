...
- 🔴 РОСТ МИРОВОЙ ЦЕНЫ НЕФТИ ДЛЯ РОССИИ — В ПЕРВУЮ ОЧЕРЕДЬ ПЛЮС, А НЕ УДАР.
  Прямые и надёжные следствия: больше нефтегазовых доходов бюджета → при прочих равных
  МЕНЬШЕ дефицит; больше валютной выручки → рубль КРЕПЧЕ → импорт дешевле, то есть канал
  ДЕЗинфляционный. «Нефть подорожала → бьёт по курсу и инфляции в РФ» — переворот знака,
  так писать нельзя. Мировая инфляция из-за нарушения цепочек поставок — отдельный, более
  слабый и СПОРНЫЙ канал: до РФ он может дойти через импорт, но это гипотеза, а рост цены
  нефти — факт. Показывай ОБЕ стороны и разделяй их по надёжности: сначала прямой эффект
  (доходы бюджета, курс, дефицит), затем оговорка о вторичном канале с пометкой «оценка».
  Внутренние цены на топливо (НПЗ, демпфер, логистика) — ТРЕТИЙ, самостоятельный канал:
  подменять им мировую цену запрещено.
