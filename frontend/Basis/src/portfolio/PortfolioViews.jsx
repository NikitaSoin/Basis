...
const AddPositionModal = ({ portfolioId, existingPositions, token, onClose, onSuccess }) => {
  ...
  // Сделка (buy/sell) НА СУЩЕСТВУЮЩЕЙ позиции — ЧЕРЕЗ /trades (record_trade),
  // не delete+recreate: (1) продажа зачисляет выручку на денежную позицию,
  // покупка симметрично списывает (владелец, 2026-07-25), (2) не плодит
  // фиктивную "buy"-транзакцию на остаток (delete+post каждый раз заводил
  // НОВУЮ открывающую сделку — портилась история для P&L/дивидендов). Только
  // для ДОБАВЛЕНИЯ К СУЩЕСТВУЮЩЕЙ позиции — создание НОВОЙ (первое "куплено")
  // по-прежнему через post(): это декларация владения, не обязательно
  // сделка прямо сейчас (портфель часто заводят задним числом), кэш её не
  // должен касаться.
  const trade = async (positionId, side, qty, prc) => check(
    await fetch(`${apiUrl}/api/portfolios/${portfolioId}/positions/${positionId}/trades`, {
      method: "POST", headers: authHeaders,
      body: JSON.stringify({ side, quantity: qty, price: prc, fee: 0, trade_date: new Date().toISOString().slice(0, 10) }),
    }),
  ...
  const handleSubmitEquity = async () => {
    ...
    if (side === "sell") {
      ...
      await trade(existing.id, "sell", qty, prc);
    } else if (existing) {
      if (!prc) throw new Error("Укажите цену покупки — по ней считается списание с денежной позиции");
      await trade(existing.id, "buy", qty, prc);
    } else {
...
const METRIC_EXPLANATIONS = {
  ...
  upside_to_fair_pct: {
    ...
    type: "judgment", icon: "🎯",
    what: "Разница между текущей рыночной ценой и справедливой ценой по методике Basis — тем же числом, что стоит в «Обзоре» карточки компании. Не факт и не прогноз, а оценочное мнение модели.",
    reading: (v, ctx = {}) => v == null ? null
...
const PortfolioV2 = ({ token, onAuthRequired, onOpenCompany, forceSection }) => {
  ...
  // позициям (stressV2Results/fetchStressV2 выше). «Чёрный лебедь» —
  ...
  //
  // ВАЖНО: этот блок должен идти ПОСЛЕ объявления stressV2Results (выше) —
  // он читает stressV2Results.rate_up/oil_crash при определении объекта, а
  // не внутри отложенного колбэка; если объявить stressMap раньше
  // useState(stressV2Results), JS кидает "Cannot access before initialization"
  // (TDZ) на КАЖДОМ рендере — ровно так уронило «Портфель» на бою 2026-07-24
  // (владелец: «Не удалось отобразить раздел, Cannot access 'Si' before
  // initialization»), билд при этом собирался чисто — эту ошибку ловит
  // только реальный запуск в браузере, не сборка.
  const stressMap = {
    black_swan: {
  ...
  if (!portfolioLoading && portfolioList.length === 0) {
    return (
      /* data-tour="portfolio-empty" — ЗАПАСНАЯ цель шага тура «Портфель»: у нового
         посетителя (а именно он и проходит экскурс) портфелей ещё нет, и сайдбар
         с разделами аналитики в этой ветке не рендерится вовсе — подсвечивать
         нечего. Проверено на бою 2026-08-05: .pf-sidebar отсутствует и через 14
         секунд. Порядок целей — в tour/tourSteps.js. */
      <div data-tour="portfolio-empty">
        {!token && (
  ...
  return (
    <div className={"pf-shell" + (sbCollapsed ? " dsc-collapsed" : "")} style={{ background: "var(--pf-tan)" }}>
      ...
      <main className="pf-main" style={{ background: "var(--pf-tan)" }}>
        <ScrollRail minCount={2} minWidth={1500} deps={[activeSection]}
