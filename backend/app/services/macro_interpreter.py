...
_DEPRECATED_SERIES = {
    ...
    # Ряды OECD MEI, которые источник прекратил вести: последние точки — 2023 и 2025 гг.
    # Без пометки модель принимает их за актуальное состояние китайской экономики.
    "cn_gdp": "ряд OECD прекращён (последняя точка 2023 г.) — не выдавай за текущий ВВП КНР",
    "cn_inflation": "ряд OECD прекращён (последняя точка 2025 г.) — не выдавай за текущую инфляцию КНР",
