...
class Resolution(str, Enum):
    ...
    RECONCILED — разбирательство ЗАКОНЧЕНО, и виновных нет: обе стороны верны,
                 но измерены по-разному. Работа не «исправить число», а «выбрать
                 одну систему измерения и пересчитать производные». Так с ЦНТЛ:
                 таблица идёт по РСБУ (сверено построчно с формой 2 из ГИР БО
                 ФНС, до рубля), а проза — по МСФО-инфографике. Числа не
                 противоречат — противоречат стандарты.

    🔴 Без этой степени такая находка висит открытой вечно: чинить в ней нечего,
    а «дефекта нет» сказать тоже нельзя — читатель видит на одной карточке две
    правды. Подсказана сессией «статус обновления данных» (11.09.2026).
    """
    LOCATED = "located"
