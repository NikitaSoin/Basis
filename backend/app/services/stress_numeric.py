...
# величины по трансмиссии ДКП, а не измерение: эффект накапливается 3-6 кварталов и
# сильнее в кредитозависимых секторах. Держим константой на виду — как и
# интенсивности сценариев, это продуктовый произвол, который должно быть можно
# оспорить, а не искать по коду.
_RATE_TO_GDP_PP = -0.35
...
def _company_numeric_impact(qi: dict, sector: str | None,
    ...
    # процентных доходов и расходов — неправильно»).
    #
    # Претензия справедлива по существу. Прямой канал описывает только обслуживание
    # долга: компания без займов получала ровно ноль, хотя при ставке 30% у неё
    # обваливается спрос. Жёсткая ставка бьёт по всей экономике — сжимается
    # потребление, останавливаются инвестиции, растёт стоимость риска.
    #
    # Поэтому шок ставки ПОРОЖДАЕТ шок спроса, и тот идёт по каналу demand, где у
    # компании уже есть собственный коэффициент чувствительности выручки к ВВП.
    # Коэффициент −0,35 п.п. ВВП на 1 п.п. ставки — оценка порядка величины по
    # трансмиссии ДКП (эффект накапливается 3-6 кварталов), а не точное измерение;
    # он произволен так же, как интенсивности сценариев, и вынесен в константу,
    # чтобы его было видно и можно было оспорить.
    #
    # Складывать с прямым каналом можно: это РАЗНЫЕ вещи — проценты по долгу и
    # выручка от спроса. Двойного счёта нет.
    if "rate" in factor_deltas and "demand" in coefs:
        gdp_shock = factor_deltas["rate"] * _RATE_TO_GDP_PP
        if abs(gdp_shock) > 1e-9:
            factor_deltas["demand"] = factor_deltas.get("demand", 0.0) + gdp_shock
            applied.append({"factor": "demand", "label": "Спрос (через ставку)",
                            "from": 0.0, "to": round(gdp_shock, 2),
                            "derived_from": "rate",
                            "assumption": (
                                f"Косвенный эффект ставки: сдвиг на "
                                f"{factor_deltas['rate']:+.1f} п.п. охлаждает экономику "
                                f"примерно на {gdp_shock:+.1f} п.п. ВВП, и это бьёт по "
                                f"выручке отдельно от процентных расходов. "
                                f"{(coefs.get('demand') or {}).get('assumption') or ''}"
                            ).strip()})
