...
🔴 СЕЙЧАС КРОН РАБОТАЕТ ТОЛЬКО В СУХОМ РЕЖИМЕ. Два прогона подряд по одному
показателю дали РАЗНЫЕ значения: пассажиропоток 30,8 (со ссылкой на AVIA.RU) и 39,5
(со ссылкой на TKS.RU). Значение, которое меняется от прогона к прогону, писать в
историю нельзя: попав в ряд, оно становится «фактом» и переживает того, кто его
добыл. Нужна сверка ВТОРЫМ ИСТОЧНИКОМ — пока её нет, крон логирует находки, а запись
включается вручную (`dry=false`) и под присмотром.
"""
