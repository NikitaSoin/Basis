...
body{font-family:Inter,-apple-system,'Segoe UI',Roboto,sans-serif;background:var(--paper);color:var(--ink);
...
h1{font-family:Fraunces,Georgia,'Times New Roman',serif;font-size:28px;line-height:1.25;margin:10px 0 4px}
h2{font-family:Fraunces,Georgia,serif;font-size:21px;margin:28px 0 10px}
h3{font-family:Fraunces,Georgia,serif;font-size:17px;margin:20px 0 8px}
...
/** Дата актуальности данных страницы.
 *
 *  🔴 НЕ БЕРЁМ meta.price_date И РОДСТВЕННЫЕ ПОЛЯ. Проверено 2026-08-08: это поле пишется
 *  ОДИН РАЗ при заведении компании и дальше не обновляется никогда. У 258 файлов из 261
 *  там лежал май-июнь, хотя сами financials.json переписываются постоянно (SBER: коммит
 *  8 августа, price_date — 16 мая). Ещё у четырёх компаний вместо даты лежал текст
 *  («из rates.csv», «P/E и P»), у одной — декабрь 2024.
 *
 *  Чем это было плохо, кроме вранья читателю: в карте сайта мы сообщаем поиску
 *  lastmod = дата сборки, а на самой странице стояло «данные на 16 мая». Расхождение
 *  «свежий lastmod при старой видимой дате» поиск читает как накрутку свежести — при
 *  том что у нас уже 229 страниц снято с вердиктом «малоценная».
 *
 *  Страница целиком пересобирается из ТЕКУЩИХ данных при каждой сборке (Timeweb гоняет
 *  npm run build у себя на каждый деплой), поэтому честная дата актуальности — дата
 *  сборки. Отчётный период при этом виден отдельно в самих таблицах и подписях. */
const BUILD_DATE = new Date().toISOString().slice(0, 10);
function companyDataDate() {
  return BUILD_DATE;
}

/* 🔴 ОТКАЧЕНО 2026-08-11 по требованию владельца — «верни обратно все страницы».
 *
 * Здесь стояло сужение индексации до состава индекса МосБиржи: страницы 218 тикеров вне
 * состава получали noindex и вылетали из карты сайта (5290 → 1068 адресов). Убрано целиком,
 * ВСЕ страницы снова индексируются. Причины отката — не вкусовые, их две, и обе фактические:
 *
 * 1. ГЕЙТ ВЫГОНЯЛ ТО, ЧТО ЯНДЕКС УЖЕ ПРИНЯЛ. На момент правки в поиске лежали 99 наших
 *    карточек, из них 32 — вне состава IMOEX: Магнит (дивиденды, отчётность, прогноз),
 *    РусГидро, Ростелеком-преф, Банк СПб-преф, Башнефть-преф, Самолёт, ДВМП. Яндекс их
 *    оставил, то есть свою проверку качества они прошли, — а мы просили их убрать.
 *    Логика «снять заранее, чтобы не снял он» к ним неприменима: он их не снимал.
 * 2. СОСТАВ ИНДЕКСА — НЕ СПИСОК «БУМАГ, КОТОРЫЕ ИЩУТ». В нём есть RTKM и BSPB, но нет
 *    RTKMP и BSPBP: обыкновенную акцию индексируем, привилегированную того же эмитента —
 *    нет, хотя ищут обе. Плюс из состава выпали MGNT и HYDR — крупные известные бумаги.
 *
 * И главное про обоснование, которое я приводил: «балласт тянул вниз живое» — ГИПОТЕЗА,
 * не факт. Вердикт LOW_QUALITY Яндекс выносит ПОСТРАНИЧНО («малоценная ИЛИ
 * маловостребованная»), и прямых данных, что страница мелкого эмитента вредит странице
 * Сбера, у нас нет. Если возвращаться к сужению — то по правилу «состав индекса ПЛЮС
 * префы тех же эмитентов ПЛЮС всё, что уже держит поиск», и с доказательством вреда,
 * а не с рассуждением о нём. */
...
// Термины живут в src/spravochnik/terms.js: их читает и этот генератор (страницы
// /pokazateli/<слаг>/ для поиска), и раздел «Справочник» внутри платформы. Один
// источник текста на двух потребителей — копия разъехалась бы при первой правке.
const TERM_PAGES = require("../src/spravochnik/terms");
...
/**
 * Разбор финансов ЧИСЛАМИ — из-за его отсутствия страница /finance/ была самой слабой:
 * 1589 знаков текста и 78% словаря одинаковы у всех компаний (замер 2026-08-05). Хуже,
 * чем было у облигаций перед тем, как Яндекс начал снимать их как «низкое качество».
 * В индексе она у нас 17 из 262, тогда как бизнес-модель (7% шаблона) — 47: поиск берёт
 * ровно уникальное.
 *
 * Всё считается из ТЕХ ЖЕ чисел, что в таблице выше: ни одной сгенерированной фразы.
 *
 * 🔴 Служебные data_flags сюда НЕ выводим. Первая версия вывела, и на страницу Сбербанка
 * попало «ИСТОРИЧЕСКИЕ МУЛЬТИПЛИКАТОРЫ — правило среднее/медиана (CV считан кодом…)» —
 * записка аналитика для аналитика: читателю ничего не объясняет, а выглядит как утечка
 * внутренней кухни.
 */
function financeInsightBlock(c) {
  const yrs = (c.years || []).map(Number);   // в объекте компании поле называется years
  const rows = finRows(c);
  const money = (v) => fmtMoney(v, c.unit, c.currency);
  const pct = (v, d = 0) => `${v > 0 ? "+" : ""}${Number(v).toLocaleString("ru-RU",
    { minimumFractionDigits: d, maximumFractionDigits: d })}%`;
  const out = [];

  // 1. Что изменилось за последний отчётный год — по каждой ключевой статье.
  const deltas = [];
  for (const r of rows) {
    const v = r.values;
    let i = v.length - 1;
    ...
    let j = i - 1;
    while (j >= 0 && v[j] == null) j--;
    if (i < 0 || j < 0 || !v[j]) continue;
    // Переход через ноль в процентах бессмыслен: «рост на 400%» из убытка в прибыль —
    // это не рост, а смена знака. Показываем словами.
    const flip = (v[j] < 0) !== (v[i] < 0);
    deltas.push({ label: r.label, from: v[j], to: v[i], flip,
                  pct: flip ? null : ((v[i] - v[j]) / Math.abs(v[j])) * 100,
                  yFrom: yrs[j], yTo: yrs[i] });
  }
  // 🔴 Сначала ПРОЗА, потом таблица. Таблица чисел почти не снижает шаблонность: подписи
  // строк («Выручка», «EBITDA») у всех одинаковы, а цифры при подсчёте общих СЛОВ не
  // различаются. Замер это подтвердил: одна таблица дала 78% → 75%. Различает страницы
  // именно фраза — с названием компании, направлением и величиной изменения.
  if (deltas.length) {
    const top = deltas[0], profit = deltas.find((d) => /прибыл/i.test(d.label)) || deltas[deltas.length - 1];
    // 🔴 БЕЗ ГЛАГОЛОВ. Первая версия писала «чистые процентные доходы вырослА на 19%»:
    // подписи статей бывают и женского рода («выручка»), и мужского («денежный поток»),
    // и множественного («процентные доходы») — согласовать их шаблоном нельзя, а
    // безграмотная фраза на витрине хуже, чем сухая. Числовая форма верна всегда.
    const dir = (d) => (d.flip ? (d.to > 0 ? "выход в прибыль" : "уход в убыток")
      : `${d.pct > 0 ? "+" : "−"}${Math.abs(Math.round(d.pct))}%`);
    const parts = [`${escapeHtml(c.short)} за ${escapeHtml(String(top.yTo || "последний год"))} год — `
      + `${escapeHtml(top.label.toLowerCase())}: ${dir(top)} (${money(top.from)} → ${money(top.to)})`];
    if (profit && profit !== top) {
      parts.push(`${escapeHtml(profit.label.toLowerCase())}: ${dir(profit)} (${money(profit.to)})`);
    }
    // Расхождение направлений — самое содержательное, что можно сказать о годе: выручка
    // растёт, а прибыль падает — значит дело в издержках, а не в спросе.
    if (profit && profit !== top && !top.flip && !profit.flip
        && Math.sign(top.pct) !== Math.sign(profit.pct)) {
      parts.push(top.pct > 0
        ? "то есть выручка прибавила, а прибыль нет — причина в расходах, а не в спросе"
        : "прибыль изменилась не так, как выручка — сработали разовые статьи или курсовые");
    }
    out.push(`<p><b>Коротко:</b> ${parts.join("; ")}.</p>`);
  }

  if (deltas.length) {
    const yF = deltas[0].yFrom, yT = deltas[0].yTo;
    out.push(`<h2>Что изменилось за последний год</h2>
<table><thead><tr><th>Показатель</th><th class="num">${escapeHtml(String(yF || "было"))}</th>
<th class="num">${escapeHtml(String(yT || "стало"))}</th><th class="num">Изменение</th></tr></thead><tbody>${
      deltas.map((d) => `<tr><td>${escapeHtml(d.label)}</td><td class="num">${money(d.from)}</td>`
        + `<td class="num">${money(d.to)}</td><td class="num">${
          d.flip ? (d.to > 0 ? "из убытка в прибыль" : "из прибыли в убыток") : pct(d.pct)}</td></tr>`).join("")}</tbody></table>`);
  }

  // 2. Оценка против СОБСТВЕННОЙ исторической нормы: видно, дорого или дёшево для этой
  // компании, а не «дёшево вообще» — у каждого бизнеса своя нормальная планка.
  const cur = ((c.fin.multiples || {}).current) || {};
  const avg = ((c.fin.multiples || {}).historical_avg) || {};
  // 🔴 Норма лежит НЕ под простым именем множителя: в данных это pe_used / pe_5y_median /
  // pe_5y_avg / pe_5y_mean — у разных компаний по-разному, потому что аналитик выбирал
  // среднее или медиану в зависимости от разброса. Первая версия читала avg.pe и не
  // находила ничего: блок молча не выводился НИ У ОДНОЙ из 264 компаний.
  const norm = (k) => {
    for (const suf of ["_used", "_5y_median", "_5y_avg", "_5y_mean"]) {
      const v = avg[k + suf];
      if (typeof v === "number" && v > 0) return v;
    }
    return null;
  };
  const pairs = [["pe", "P/E"], ["pb", "P/B"], ["ps", "P/S"], ["ev_ebitda", "EV/EBITDA"]]
    .map(([k, label]) => {
      const a = cur[k], b = norm(k);
      return (typeof a === "number" && a > 0 && b) ? { label, a, b, diff: ((a - b) / b) * 100 } : null;
    }).filter(Boolean);
  if (pairs.length) {
    const d0 = pairs[0].diff;
    out.push(`<h2>Дорого или дёшево по своим меркам</h2>
<p class="sub">Текущие множители против исторической нормы самой компании. Это честнее
сравнения с рынком: у каждого бизнеса своя планка, и «дёшево относительно сектора» часто
означает лишь, что сектор дорогой.</p>
<table><thead><tr><th>Множитель</th><th class="num">Сейчас</th><th class="num">Своя норма</th>
<th class="num">Отклонение</th></tr></thead><tbody>${pairs.map((p) =>
      `<tr><td>${p.label}</td><td class="num">${p.a.toLocaleString("ru-RU", { maximumFractionDigits: 2 })}</td>`
      + `<td class="num">${p.b.toLocaleString("ru-RU", { maximumFractionDigits: 2 })}</td>`
      + `<td class="num">${pct(p.diff)}</td></tr>`).join("")}</tbody></table>
<p class="sub">${d0 < -15
      ? `${escapeHtml(c.short)} торгуется ниже собственной исторической нормы. Это повод разобраться, а не вывод: рынок мог переоценить риски, а мог увидеть то, чего в отчёте пока нет.`
      : d0 > 15
        ? `${escapeHtml(c.short)} торгуется выше собственной исторической нормы — рынок закладывает улучшение, которого в отчётности ещё не видно.`
        : `Оценка близка к исторической норме компании — по множителям бумага не выглядит ни дорогой, ни дешёвой относительно своего прошлого.`}</p>`);
  }
  return out.join("\n");
}
...
<table><thead><tr><th>Компания</th><th>Потенциал</th></tr></thead><tbody>${
  ...
  const title = `${titleName(c)} (${c.ticker}): справедливая цена акций — расчёт и потенциал | Basis`;
...
function metricPage(c, spec, pts, assets, tabsWritten, sectorAll) {
  ...
  // 🔴 Владелец 2026-08-06: «если клиент ищет "сбер дивиденды", название страницы должно
  // быть "сбер дивиденды"». У банков статья выручки называется «чистые процентные доходы»,
  // и слова «выручка» в заголовке не было ВООБЩЕ — по массовому запросу «выручка сбербанка»
  // страница не совпадала ни одним словом. Показываем оба: сначала то, что ищут, в скобках —
  // как это называется в банковской отчётности.
  const searched = (spec.slug === "vyruchka" && c.profile === "bank")
    ? `выручка (${short.toLowerCase()})` : short;
  const title = `${titleName(c)} (${c.ticker}): ${searched} по годам — ${fmt(last.value)} за ${last.year} | Basis`;
...
function writeSitemap(urls) {
  // Отсев бумаг вне состава IMOEX стоял здесь и откачен 2026-08-11 вместе с гейтом в
  // pageShell — причины в комментарии там же. В карту сайта снова идёт всё.
