...
Строки «Выручка/EBITDA нефтебизнеса» — financials.json (единый источник чисел
карточки), консолидированная МСФО-отчётность; остальные строки таблицы (доход
от «кубышки», переоценка, чистая прибыль РСБУ, дивиденд) — оценка на базе РСБУ,
которая формирует дивидендную базу префа и в financials.json не заведена
отдельным рядом. Вывод из таблицы: дивиденд SNGSP привязан уставом к чистой
прибыли по РСБУ (~7,1%), но сама прибыль волатильна. Номинально она формируется
из (1) EBITDA нефтебизнеса, (2) финансового дохода от «кубышки» (проценты по
депозитам) и (3) курсовой переоценки валютных активов. При ослаблении рубля
дивиденд SNGSP растёт (переоценка добавляет в прибыль); при укреплении рубля —
падает или обращается в убыток (переоценка съедает прибыль). Именно поэтому
SNGSP часто называют валютным хеджем для российского инвестора.
