"""Скрининг облигаций — все бумаги с вердиктом «доходность vs риск» (методика
...
# Кеш результата (TTL) — расчёт по ~3177 бумагам тяжеловат для каждого запроса; данные
# меняются только при обновлении котировок/деплое. Прогревается при старте (warm_bonds_cache).
