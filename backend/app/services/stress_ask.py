...
Верни СТРОГО JSON:
{
  ...
  "out_of_scope": <true, если сценарий про ТОЧЕЧНОЕ событие одной конкретной компании
    (адресный налог на неё, смена её собственника, авария на её заводе) — такое наша
    факторная модель не считает; иначе false>,
...
def ask_scenario(db: Session, question: str) -> dict:
    ...
    if "numeric" not in result and "qualitative" not in result:
        result["no_signal"] = True
        result["note"] = ("Из сценария не удалось извлечь ни числовых уровней, ни качественных "
                         "факторов — уточните формулировку (например: «ставка 20%», «нефть $50», "
                         "«война затягивается», «налоги на бизнес растут»).")
    return result
